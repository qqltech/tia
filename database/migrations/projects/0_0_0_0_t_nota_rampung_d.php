...
class tnotarampungd extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            $table->id()->from(1);
            $table->integer('t_nota_rampung_id')->comment('{"fk":"t_nota_rampung.id"}')->nullable();
            $table->integer('t_buku_order_d_npwp_id')->comment('{"src":"t_buku_order_d_npwp.id"}');
            $table->string('no_kontainer',30)->nullable();
