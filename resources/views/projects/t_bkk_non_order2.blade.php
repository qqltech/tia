    ...
    <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
      ...
      <div>
        <FieldX :bind="{ readonly: true }" class="w-full !mt-3"
          :value="data.no" :errorText="formErrors.no?'failed':''"
          @input="v=>data.no=v" :hints="formErrors.no" 
          label="No. Draft"
      ...
      <div>
        <FieldX :bind="{ disabled: true, readonly: true }" class="w-full !mt-3 pointer-events-none"
          :value="data.tanggal" :errorText="formErrors.tanggal?'failed':''"
          :hints="formErrors.tanggal" 
          :check="false"
          type="date"
          label="Tgl BKK"
          placeholder="Pilih Tgl BKK"
        />
      </div>
      <div>
        <FieldX :bind="{ readonly: true }" class="w-full !mt-3"
          :value="data.no_bkk_non_order" :errorText="formErrors.no_bkk_non_order?'failed':''"
          @input="v=>data.no_bkk_non_order=v" :hints="formErrors.no_bkk_non_order" 
          label="BKK (Non Buku Order)"
          placeholder="BKK (Non Buku Order)"
          :check="false"
        />
      </div>
      <div>
        <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3"
          :value="data.no_reference" :errorText="formErrors.no_reference?'failed':''"
          @input="v=>data.no_reference=v" :hints="formErrors.no_reference" 
          label="No. Reference"
      ...
      <div>
        <FieldNumber :bind="{ readonly: true }" class="w-full !mt-3"
          :value="data.total_amt" :errorText="formErrors.total_amt?'failed':''"
          @input="v=>data.total_amt=v" :hints="formErrors.total_amt" 
          label="Total Amt"
      ...
      <div>
        <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3"
          type="textarea"
          :value="data.catatan" :errorText="formErrors.catatan?'failed':''"
          @input="v=>data.catatan=v" :hints="formErrors.catatan" 
          label="Catatan"
          placeholder="Catatan"
          :check="false"
        />
            ...
            <tr v-for="(item, i) in detailArr" :key="item.id" class="border-t" v-if="detailArr.length > 0">
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldNumber :bind="{ disabled: !actionText, clearable:false }"
                  class="w-full py-2 !mt-0" :value="item.nominal" @input="v=>item.nominal=v"
                  :errorText="formErrors.nominal?'failed':''" :hints="formErrors.nominal"
                />
              </td>
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ disabled: !actionText, clearable:false }"
                  class="w-full py-2 !mt-0" :value="item.note" @input="v=>item.note=v"
                  :errorText="formErrors.note?'failed':''" :hints="formErrors.note" type="textarea"
                />
              </td>
              <td class="p-2 border border-[#CACACA]">
