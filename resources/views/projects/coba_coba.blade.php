  ...
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable: true }" class="w-full !mt-3" :value="values.tipe_order"
        ...
        :hints="formErrors.tipe_order" valueField="key" displayField="key" :options="['EKSPORT', 'EKSPORT S', 'IMPORT', 'IMPORT S', 'LOKAL', 'OL', 'OLS']"
        placeholder="Pilih Tipe Order" label="Tipe Order" :check="true" />
    </div>
    <div>
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="values.kode_cust"
        :errorText="formErrors.kode_cust?'failed':''" @input="v=>values.kode_cust=v"
        :hints="formErrors.kode_cust" label="Kode Customer" placeholder="Kode Customer" :check="false" />
    </div>
    <div>
    ...
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.jenis_barang"
        :errorText="formErrors.jenis_barang?'failed':''" @input="v=>values.jenis_barang=v" :hints="formErrors.jenis_barang" :check="false"
        label="Jenis Barang" placeholder="Tuliskan Jenis Barang"/>
    </div>
    <div>
    ...
    <div>
      <FieldUpload :bind="{ readonly: !actionText }"
          :value="values.berkas_coo" @input="(v)=>values.berkas_coo=v" :maxSize="10"
          ...
          :api="{
            ...
            onerror:(error)=>{},
          }" class="w-full !mt-3" label="Berkas COO"
          :hints="formErrors.berkas_coo" placeholder="Berkas COO" fa-icon="upload"
          :accept="acceptType('berkas_coo')" :check="false"  
        />
    </div>
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.kode_pelayaran_id"
        @input="v=>{
            if(v){
          ...
          }" :errorText="formErrors.kode_pelayaran_id?'failed':''" :hints="formErrors.kode_pelayaran_id" valueField="id"
        displayField="deskripsi" :api="{
          ...
          }" placeholder="Pilih Kode Pelayaran" label="Kode Pelayaran" :check="true" />
    </div>
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.nama_pelayaran"
        :errorText="formErrors.nama_pelayaran?'failed':''" @input="v=>values.nama_pelayaran=v"
        :hints="formErrors.nama_pelayaran" :check="false" label="Nama Pelayaran"
    ...
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.lokasi_stuffing"
        :errorText="formErrors.lokasi_stuffing?'failed':''" @input="v=>values.lokasi_stuffing=v" :hints="formErrors.lokasi_stuffing" :check="false"
        label="Lokasi Stuffing" placeholder="Tuliskan Lokasi Stuffing" />
    </div>
    <div>
    ...
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.tipe"
        @input="v=>{
            if(v){
              values.tipe=v
            }else{
    ...
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.note"
        :errorText="formErrors.note?'failed':''" @input="v=>values.note=v" :hints="formErrors.note" :check="false"
        label="Catatan" type="textarea" placeholder="Tuliskan Catatan" />
    </div>
    <div>
  ...
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2" v-if="activeTabIndex === 0">
    ...
    <div>
      <FieldSelect :bind="{ disabled: !actionText, clearable:true }" class="w-full !mt-3" :value="values.ukuran" @input="v=>{
          ...
          }" :errorText="formErrors.ukuran?'failed':''" :hints="formErrors.ukuran" valueField="deskripsi"
        displayField="deskripsi" :api="{
              url: `${store.server.url_backend}/operation/m_general`,
              headers: { 'Content-Type': 'Application/json', Authorization: `${store.user.token_type} ${store.user.token}`},
              params: {
          ...
          <tr v-for="(item, i) in detailArr" :key="item.id" class="border-t" v-if="detailArr.length > 0">
            ...
            <td class="p-2 border border-[#CACACA]">
              <FieldX :bind="{ readonly: true }" class="w-full py-2 !mt-0" :value="item.no_buku_order"
                :errorText="formErrors.no_buku_order?'failed':''" @input="v=>item.no_buku_order=v"
                :hints="formErrors.no_buku_order" :check="false" label="" placeholder="No. Buku Order" />
            </td>
            <td class="p-2 border border-[#CACACA]" style="min-width: 100px !important;">
              <FieldSelect
                :bind="{ disabled: !actionText, clearable:true }" class="w-full py-2 !mt-0"
  ...
  <div class="<md:col-span-1 col-span-3 p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 " v-if="activeTabIndex === 2">
    ...
    <div class="overflow-x-auto <md:col-span-1 col-span-3">
      <table class="w-[150%] lg:w-full overflow-x-auto table-auto border border-[#CACACA] mt-4">
        ...
        <tbody>
          <tr v-for="(itemBerkas, i) in detailArrBerkas" :key="itemBerkas.id" class="border-t" v-if="detailArrBerkas.length > 0">
            <td class="p-2 text-center border border-[#CACACA]">
              {{ i + 1 }}.
            </td>
            <td class="p-2 border border-[#CACACA]">
              <FieldX :bind="{ readonly: !actionText }" class="w-full py-2 !mt-0" :value="itemBerkas.nama_berkas"
                :errorText="formErrors.nama_berkas?'failed':''" @input="v=>itemBerkas.nama_berkas=v" :hints="formErrors.nama_berkas"
                :check="false" label="" placeholder="Nama Berkas" />
            </td>
            <td class="p-2 border border-[#CACACA]">
              <FieldUpload :bind="{ readonly: !actionText }"
                :value="itemBerkas.foto_berkas" @input="(v)=>itemBerkas.foto_berkas=v" :maxSize="10"
                :reducerDisplay="val=>!val?null:val.split(':::')[val.split(':::').length-1]"
                :api="{
                  url: `${store.server.url_backend}/operation/t_buku_order/upload`,
                  headers: { Authorization: `${store.user.token_type} ${store.user.token}`},
                  ...
                  onsuccess: response=>response,
                  onerror:(error)=>{},
                }" class="w-full py-2 !mt-0" label=""
                :hints="formErrors.foto_berkas" placeholder="" fa-icon="upload"
                accept="*" :check="false"  
              />
            </td>
            <td class="p-2 border border-[#CACACA]">
