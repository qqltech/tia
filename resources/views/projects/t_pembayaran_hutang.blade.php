...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
  ...
  <div class="grid <md:grid-cols-1 grid-cols-3 grid-flow-row p-4 gap-3">
    ...
    <div class="w-full !mt-3">
      <FieldSelect class="!mt-0" :bind="{  disabled:!actionText , readonly: !actionText }" displayField="deskripsi" valueField="id" 
        :value="values.tipe_pembayaran_id" @input="(v)=>values.tipe_pembayaran_id=v"
        :errorText="formErrors.tipe_pembayaran_id?'failed':''" :hints="formErrors.tipe_pembayaran_id"
        placeholder="Tipe Pembayaran" label="Tipe Pembayaran" :check='false' :api="{
    ...
    <div class="w-full !mt-3">
      <FieldPopup class="!mt-0" displayField="no_rph" valueField="id" :bind="{ readonly: !actionText }"
        :value="values.t_rencana_pembayaran_hutang_id" @input="(v)=>values.t_rencana_pembayaran_hutang_id=v" :errorText="formErrors.t_rencana_pembayaran_hutang_id?'failed':''"
        :hints="formErrors.t_rencana_pembayaran_hutang_id" 
        placeholder="Pilih Rencana Pembayaran Hutang" 
        label="Rencana Pembayaran Hutang" 
        :check='false'
        @update:valueFull="rencana_PH" 
        :api="{
      ...
      <FieldPopup class="!mt-0" 
        ...
        valueField="m_supplier_id" :bind="{ readonly: !values.t_rencana_pembayaran_hutang_id  || !actionText}"
        :value="values.supplier_id" @input="(v)=>values.supplier_id=v" :errorText="formErrors.supplier_id?'failed':''"
        :hints="formErrors.supplier_id" placeholder="Pilih Supplier" label="Supplier" :check='false' 
...
<div class="justify-items-start text-xl ml-10">
  <h1 class="font-semibold italic">Total Dibayar = <soan class="text-red-500">{{ formatRupiah(values.total_amt) }}</span> </h1>
</div>
              ...
              <tbody>
                ...
                <tr v-else v-for="(item, index) in detailArr" :key="index" class="border">
                  ...
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldX class="!mt-0" :bind="{ readonly: true }" :value="item.no_pi"
                      @input="(v)=>item.no_pi=v" :errorText="formErrors.no_pi?'failed':''"
                      :hints="formErrors.no_pi" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldX class="!mt-0" :bind="{ readonly: true }" :value="item.tgl_pi"
                      @input="(v)=>item.tgl_pi=v" :errorText="formErrors.tgl_pi?'failed':''"
                      :hints="formErrors.tgl_pi" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldX class="!mt-0" :bind="{ readonly: true }" :value="item.tgl_jt"
                      @input="(v)=>item.tgl_jt=v" :errorText="formErrors.tgl_jt?'failed':''"
                      :hints="formErrors.tgl_jt" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                  ...
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldNumber class="!mt-0" :bind="{ readonly: !actionText }" :value="item.bayar" @input="(v)=>item.bayar=v"
            ...
            <table class="w-[150%] lg:w-full overflow-x-auto table-auto border border-[#CACACA] mt-4">
              ...
              <tbody>
                ...
                <tr v-else v-for="(item, index) in detailArr1" :key="index" class="border">
                  ...
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldX class="!mt-0" :bind="{ readonly: true }" :value="item.supplier"
                      @input="(v)=>item.supplier=v" :errorText="formErrors.supplier?'failed':''"
                      :hints="formErrors.supplier" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldX class="!mt-0" :bind="{ readonly: true }" :value="item.tgl_jt"
                      @input="(v)=>item.tgl_jt=v" :errorText="formErrors.tgl_jt?'failed':''"
                  ...
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldNumber class="!mt-0" :bind="{ readonly: true }" :value="item.jumlah"
                      @input="(v)=>item.jumlah=v" :errorText="formErrors.jumlah?'failed':''"
                      :hints="formErrors.jumlah" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldNumber class="!mt-0" :bind="{ readonly: true}" :value="item.jumlah_bayar" @input="(v)=>item.jumlah_bayar=v"
                      :errorText="formErrors.jumlah_bayar?'failed':''" :hints="formErrors.jumlah_bayar" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                  ...
                  <td class="p-2 border border-[#CACACA] text-center max-w-52 truncate">
                    <FieldX :bind="{ readonly: true}" :value="item.tgl_realisasi"
                      :errorText="formErrors.tgl_realisasi?'failed':''" @input="v=>item.tgl_realisasi=v" :hints="formErrors.tgl_realisasi"
                      placeholder="Tanggal Realisasi" label="" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
                    <FieldNumber :bind="{ readonly: true }" :value="item.jumlah_realisasi" @input="(v)=>item.jumlah_realisasi=v"
                      :errorText="formErrors.jumlah_realisasi?'failed':''" :hints="formErrors.jumlah_realisasi"
                      placeholder="Jumlah Realisasi" label="" :check="false" />
                  </td>
                  <td class="p-2 text-center border border-[#CACACA]">
