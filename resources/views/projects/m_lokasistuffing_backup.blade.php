  ...
  <div class="bg-blue-500 text-white rounded-t-md pt-2 pb-3 px-4">
    <div class="flex items-center gap-2">
      ...
      <div class="flex flex-col py-1 gap-1">
        <h1 class="text-lg font-bold leading-none">Form Lokasi Stuffing</h1>
    ...
    <div class="pt-2 pb-4 px-4 grid grid-cols-2 gap-y-2 gap-x-4 sticky">
        <div class="relative">
          <label v-show="data.kode_lokasi === '' || !data.kode_lokasi" 
            class="absolute top-2 left-1 left text-gray-600 text-xs font-semibold" >Kode</label>
          <FieldX :bind="{ readonly: true || !actionText }" class="pt-1" :value="data.kode_lokasi"
            :errorText="formErrors.kode_lokasi?'failed':''" @input="v=>data.kode_lokasi=v" :hints="formErrors.kode_lokasi" label="Kode" 
            placeholder='Auto Generate by System' :check="false" />
        </div>
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.nama_lokasi"
          :errorText="formErrors.nama_lokasi?'failed':''" @input="v=>data.nama_lokasi=v" :hints="formErrors.nama_lokasi" 
        ...
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.alamat" :errorText="formErrors.alamat?'failed':''" 
          @input="v=>data.alamat=v" :hints="formErrors.alamat" 
