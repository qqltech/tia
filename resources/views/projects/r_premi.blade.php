  ...
  <div class="flex gap-x-4 px-2">
    <div class="flex flex-col border rounded shadow-sm <md:w-full w-full bg-white">
      <div class="bg-blue-500 text-white rounded-t-md py-2 px-4">
        <div class="flex items-center">
          <Icon fa="arrow-left" class="cursor-pointer mr-2 font-bold hover:text-yellow-500" title="Kembali"
            @click="onBack" />
          <div>
            <h1 class="text-20px font-bold mb-4 mt-4">Laporan Premi</h1>
          </div>
        </div>
      </div>
      <hr>
      <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
        ...
        <div>
          <FieldSelect :bind="{ readonly: !actionText, clearable: false }" class="w-full !mt-3" :value="values.supir_id"
            :errorText="formErrors.supir_id ? 'failed' : ''" @input="v => {
              values.supir_id = v;
              }" :hints="formErrors.supir_id" valueField="id" displayField="nama" :api="{
              ...
              }" placeholder="Pilih Supir" label="Supir" :check="true" />
        </div>
        <div>
