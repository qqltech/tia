...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
  <div class="bg-blue-500 text-white rounded-t-md py-2 px-4">
    <div class="flex items-center">
      ...
      <div>
        <h1 class="text-20px font-bold">Form Transaksi Debit Note</h1>
        <p class="text-gray-100">Form untuk Transaksi Debit Note</p>
      </div>
    </div>
  </div>
  <!-- HEADER END -->
  ...
  <div class="grid <md:grid-cols-1 grid-cols-3 grid-flow-row p-4 gap-3">
    ...
    <div class="w-full !mt-3">
      <FieldX class="!mt-0" :bind="{ readonly: true }" :value="values.no_debit_note"
        :errorText="formErrors.no_debit_note?'failed':''" @input="v=>values.no_debit_note=v"
        :hints="formErrors.no_debit_note" placeholder="No Debit Note" label="No Debit Note" :check="false" />
    </div>
    ...
    <div class="w-full !mt-3">
      <FieldSelect class="!mt-0" :bind="{ disabled: !actionText, readonly: !actionText, clearable: false }"
        displayField="deskripsi" valueField="id" :value="values.tipe_debit_note"
        @input="(v) => values.tipe_debit_note = v" :errorText="formErrors.tipe_debit_note ? 'failed' : ''"
        :hints="formErrors.tipe_debit_note" placeholder="Tipe Debit Note" label="Tipe Debit Note" :check='false'
        @update:valueFull="(data)=>{
         values.supplier_id = null;
         values.customer_id = null;
         values.perkiraan_debit =  null;
         detailArr = [];
         subDetail = [];
         //detailArr2 = [];
      ...
      params: {
        join: true,
        simplest: true,
        where: `this.is_active=true and this.group='TIPE DEBIT NOTE'`,
    ...
    <div class="w-full !mt-3">
      <FieldPopup v-show="values.tipe_cn === 'HUTANG'" class="!mt-0" displayField="nama" valueField="id"
        ...
        label="Supplier" :check='false' @update:valueFull="(data)=>{
         detailArr = [];
         subDetail = [];
         //detailArr2 = [];
         values.perkiraan_debit = null;
    ...
    },
    {
      ...
      <FieldPopup v-show="values.tipe_cn === 'PIUTANG'" class="!mt-0" displayField="nama_perusahaan" valueField="id"
        :bind="{ readonly: !actionText }" :value="values.customer_id" @input="(v) => values.customer_id = v"
        :errorText="formErrors.customer_id ? 'failed' : ''" :hints="formErrors.customer_id" placeholder="Pilih Customer"
        label="Customer" :check='false' @update:valueFull="(data)=>{
         detailArr = [];
         subDetail = [];
         //detailArr2 = [];
         values.perkiraan_debit = null;
    ...
    <div class="w-full !mt-3">
      <FieldPopup class="!mt-0" displayField="nama_coa" valueField="id" :bind="{ readonly: !actionText }"
        :value="values.perkiraan_debit" @input="(v)=>values.perkiraan_debit=v"
        :errorText="formErrors.perkiraan_debit?'failed':''" :hints="formErrors.perkiraan_debit"
        placeholder="Pilih Perkiraan Debit" label="Perkiraan Debit" :check='false' :api="{
    ...
    <div class="w-full !mt-3 ">
      <FieldNumber class="!mt-0" :bind="{ readonly: true, disabled: true }" type="number" :value="totalDebitNote"
        @input="(v)=>values.total_debit_note=v" :errorText="formErrors.total_debit_note?'failed':''"
        :hints="formErrors.total_debit_note" :check="false" placeholder="Total Debit Note" label="Total Debit Note" />
    </div>
  ...
  <div class="p-4 space-x-5 text-sm text-white">
    <button v-if="values.tipe_credit_note == 317"
  ...
  <div v-show="activeTab === 1">
    <div class="<md:col-span-1 col-span-3 p-4 grid <md:grid-cols-1 grid-cols-3 gap-2">
      ...
      <ButtonMultiSelect v-show="values.tipe_cn == 'HUTANG' && actionText" title="Add to list" @add="onDetailAdd" :api="{
            ...
            params: { 
              //scopes: 'Detail',
              where: `this.m_supplier_id = ${values.supplier_id}`, 
        ...
        <table v-if="values.tipe_cn == 'HUTANG'"
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in detailArr" :key="index" class="border">
              <td class="p-2 text-center border border-[#CACACA]">{{ item.no_urut }}</td>
              ...
              <td class="p-2 text-center border border-[#CACACA]">
                <FieldNumber :bind="{ readonly: true, disabled: true }" :value="subTotal2(item)"
                  ...
                  :hints="formErrors.sub_total_amount" placeholder="" label="" :check="false" />
              </td>
  ...
  <div class="p-4" v-show="activeTab === 2">
    ...
    <div v-show="values.tipe_cn == 'HUTANG'" class="p-4 flex flex-row  justify-center items-center w-[35%]">
      ...
      <!-- <FieldPopup class="!mt-0" displayField="t_purchase_invoice.no_pi" valueField="id"
        ...
        :api="{
            url: `${store.server.url_backend}/operation/t_debit_note_d`,
            headers: {
              'Content-Type': 'Application/json',
              Authorization: `${store.user.token_type} ${store.user.token}`
            },
    ...
    <div class="">
      <ButtonMultiSelect v-if = "actionText" title="Add to list" @add="onAddSubDetail" :api="{
            url: `${store.server.url_backend}/operation/m_coa`,
            headers: { 'Content-Type': 'Application/json', Authorization: `${store.user.token_type} ${store.user.token}`},
            params: { 
              where:`this.is_active = true and this.debit_kredit = 'DEBIT'`
            },
            onsuccess:(response)=>{
              response.page = response.current_page
              response.hasNext = response.has_next
          ...
          {
            flex: 1,
            headerName:'Tipe Perkiraan Debit',
            sortable: false, resizable: true, filter: 'ColFilter',
        ...
        <table class="w-[150%] lg:w-full overflow-x-auto table-auto border border-[#CACACA] mt-4">
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in subDetail" :key="index" class="border">
              ...
              <td class="p-2 text-center border border-[#CACACA]">
                <FieldNumber class="!mt-0" :bind="{ readonly: !actionText, disabled: !actionText }" :value="item.amount" @input="(v)=>item.amount=v"
                  :errorText="formErrors.amount?'failed':''" :hints="formErrors.amount" :check="false" />
              </td>
              <td class="p-2 border border-[#CACACA] text-center max-w-52 truncate">
                <FieldX :bind="{ readonly: !actionText, disabled: !actionText  }" :value="item.catatan"
