...
// Ambil data dari r_gl dengan filter kas masuk (credit entries)
$data = \DB::select("
    SELECT 
        rgl.id,
        rgl.date as tanggal,
        ...
        rgl.status,
        rgl.no_reference,
        SUM(rgld.credit) as total_amt
    FROM r_gl rgl
    LEFT JOIN r_gl_d rgld ON rgl.id = rgld.r_gl_id AND rgld.delete_at IS NULL
    LEFT JOIN m_customer c ON rgl.m_cust_id = c.id
    LEFT JOIN m_supplier s ON rgl.m_supp_id = s.id
    WHERE {$whereClause} 
        AND rgl.status = 'POST' 
        AND rgl.delete_at IS NULL
        AND rgld.credit > 0
    GROUP BY rgl.id, rgl.date, rgl.type, rgl.ref_table, rgl.ref_id, rgl.ref_no, 
             rgl.desc, rgl.status, rgl.no_reference, rgl.m_cust_id, rgl.m_supp_id
...
<div style="margin: 20px 0;">
    ...
    <table class="cash-in-report-table">
        ...
        <tbody>
            @foreach ($data as $dt)
            @php
            // Ambil detail GL berdasarkan r_gl_id untuk credit entries (kas masuk)
            $detail = \DB::select("
                SELECT rgld.*, 
            ...
            @if ($span > 0)
                ...
                <tr>
                    ...
                    <td class="amount-cell" style="color: #17a2b8; font-weight: bold;">
            ...
            <tr>
                <td colspan="10" class="no-data" style="padding: 30px;">
                    Tidak ada data General Ledger untuk kas masuk pada periode ini
                </td>
            </tr>
            @endif
            ...
            <tr class="grand-total">
                ...
                <td class="amount-cell" style="font-size: 14px; color: #17a2b8;">
    ...
    <div style="margin-top: 20px; padding: 15px; background-color: #d1ecf1; border-left: 4px solid #17a2b8;">
        <h4 style="margin: 0; color: #0c5460;">Ringkasan General Ledger - Kas Masuk</h4>
        <p style="margin: 5px 0; color: #0c5460;">
            Total Transaksi: <strong>{{ count($data) }}</strong> | 
            Total Kas Masuk: <strong style="color: #17a2b8;">Rp {{ number_format($grand_total, 2, ',', '.') }}</strong>
        </p>
        <p style="margin: 5px 0; font-size: 12px; color: #666;">
            <em>Data diambil dari tabel General Ledger (r_gl) dengan filter credit entries untuk kas masuk</em>
        </p>
    </div>
    @endif
</div>
