...
$dataBkmNonOrder = \DB::select('select
*
from t_bkm_non_order tbno
where tbno.id = ?
',[$req['id']]);

$detailBkmNonOrder = \DB::select('select
*,
tbnod.keterangan as tbnod_keterangan,
mc.nama_coa,
mc.nomor as nomor_coa
from t_bkm_non_order_d tbnod
left join m_coa mc on mc.id = tbnod.m_coa_id
where tbnod.t_bkm_non_order_id = ?
...
$numbering = 1;
$total_nominal = 0;
foreach($detailBkmNonOrder as $dbno){
...
<body class="continuous_form">
  <section class="sheet padding-3mm" style="padding-right: 30px;">
    <table style="width: 100%; font-size: 12px;">
        <tr>
          <!-- {{var_dump($n)}} -->
          <td style="text-align: center; font-family: sans-serif; font-weight: bold; padding: 2%;">BKM NON ORDER</td>
        </tr>
      </table>
<!-- Tabel 1 -->
    <table style="width:100%; border: 0.5px solid black; line-height: 1.5;">
      ...
      <tr>
        <td style="width: 60%%;">No. Ref
          <span style="margin-left: 6.7%;">:</span>
          <span style="margin-left: 5%;">{{$n->no_ref}}</span>
        </td>
        ...
        <td colspan="3"
          ...
          <table cellspacing="0" cellpadding="3" border="0.5"
            ...
            <tr>
              ...
              <td style="width: 22%; text-align: center; border: 0.5px solid black;">{{$dbno->tbnod_keterangan}}</td>
