...
<div class="bg-white rounded-md min-h-[520px] border-t-10 border-blue-500">
  <div class="pl-4 pt-2 pb-2">
    <h1 class="text-xl font-semibold">PREMI BATAL</h1>
  </div>
...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
  <div class="bg-blue-500 text-white rounded-t-md pt-2 pb-3 px-4">
    <div class="flex items-center gap-2">
      ...
      <div class="flex flex-col py-1 gap-1">
        <h1 class="text-lg font-bold leading-none">Form Premi Batal</h1>
        <p class="text-gray-100 leading-none">Transaction Premi Batal</p>
      </div>
    </div>
  </div>
  <!-- HEADER -->
  <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 items-start">
    <!-- col-span-2 -->
    <div>
      <FieldX class="w-full !mt-3" :bind="{ readonly: true }" :value="data.no_premi_batal"
        :errorText="formErrors.no_premi_batal?'failed':''" @input="v=>data.no_premi_batal=v"
        :hints="formErrors.no_premi" placeholder="Generated by System" label="No Premi Batal" :check="false" />
    </div>
    <div>
      <FieldPopup class="w-full !mt-3" :api="{
        ...
        params: {
          simplest:false,
          searchfield: 'this.id, this.no_spk, this.tipe_spk, supir.nama, sektor1.deskripsi, this.sangu',
        },
        onsuccess: (response) => {
          return response;
        }
        ...
        if(res){
          ...
          data.waktu_out = res['waktu_out.deskripsi'];
          data.waktu_in = res['waktu_in.deskripsi'];
          data.no_bon_sementara = res.no_bon_sementara;
          data.tanggal_bon = res.tanggal_bon;
          data.head_deskripsi = res['head.deskripsi']
    ...
    <div>
      <FieldSelect class="w-full !mt-3" :bind="{ disabled: true, clearable:false }" :value="data.status"
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3 pointer-events-none" :value="data.tgl"
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.no_premi_batal"
        @input="v=>data.no_premi_batal=v" :errorText="formErrors.no_premi_batal?'failed':''"
        :hints="formErrors.no_premi_batal" label="No. Premi" placeholder="No. Premi" :check="false" />
    </div>
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3 pointer-events-none" :value="data.tgl_premi"
        :errorText="formErrors.tgl_premi?'failed':''" @input="v=>data.tgl_premi=v" :hints="formErrors.tgl_premi"
        :check="false" type="date" label="Tanggal Premi" placeholder="Tanggal Premi" />
    </div>
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.no_container" @input="v=>data.no_container=v"
        ...
        placeholder="No. Container" :check="false" />
    </div>
    <div class="grid grid-cols-2 gap-y-2 gap-x-2 items-start">
      <FieldX :bind="{ disabled: true, readonly: true }" class="w-full !mt-3" :value="data.tanggal_out"
        ...
        :check="false" type="date" label="Tanggal Out" placeholder="Tanggal Out" />

      <FieldSelect class="w-full !mt-3 pointer-events-none" :bind="{ disabled: true, readonly: true, clearable: false }"
        :value="data.waktu_out" @input="v=>data.waktu_out=v" :errorText="formErrors.waktu_out?'failed':''"
        :hints="formErrors.waktu_out" valueField="id" displayField="waktu_out.deskripsi" :options="[{'id' : 'Pagi', 'key' : 'Pagi'},
      {'id' : 'Siang', 'key' : 'Siang'},
      {'id' : 'Sore', 'key' : 'Sore'}]" placeholder="Waktu Out" label="Waktu Out" :check="false" />
    </div>
    <div></div>
    <div>
    ...
    <div class="grid grid-cols-2 gap-y-2 gap-x-2 items-start">
      ...
      <FieldSelect class="w-full !mt-3 pointer-events-none" :bind="{ disabled: true, readonly: true, clearable: false }"
        ...
        :hints="formErrors.waktu_in" valueField="id" displayField="waktu_in.deskripsi" :options="[{'id' : 'Pagi', 'key' : 'Pagi'},
      {'id' : 'Siang', 'key' : 'Siang'},
      {'id' : 'Sore', 'key' : 'Sore'}]" placeholder="Waktu In" label="Waktu In" :check="false" />
    </div>
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.head_deskripsi"
        @input="v=>data.head_deskripsi=v" :errorText="formErrors.head_deskripsi?'failed':''"
        :hints="formErrors.head_deskripsi" label="No. Head" placeholder="No. Head" :check="false" />
    </div>
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.ukuran_container"
        ...
        :hints="formErrors.ukuran_container" label="Ukuran Container" placeholder="Ukuran Container" :check="false" />
    </div>
    <div>
      <FieldPopup class="w-full !mt-3" :api="{
  ...
  <!-- START TABLE DETAIL -->
  <div class="p-2 ml-2 text-xl font-bold">
    <h1>Premi Tambahan Detail</h1>
  </div>
  <div class="<md:col-span-1 col-span-3 p-2 grid <md:grid-cols-1 grid-cols-3 gap-2 ml-2">
    <div class="overflow-x-auto <md:col-span-1 col-span-3">
      <button class="text-xs rounded py-1.5 px-1.5 text-white bg-blue-600 hover:bg-blue-700 flex gap-x-1
  ...
  <!-- END TABLE DETAIL -->
  <div class="grid grid-cols-3 gap-y-2 gap-x-2 items-start justify-center w-md mb-3 ml-2">
    <label class="!mt-4 !ml-3"> Total Premi </label>
    <FieldNumber :bind="{ readonly: true }" class="w-full content-center !mt-3 col-span-2" :value="data.total_premi"
