...
<div class="bg-white p-1 rounded-md min-h-[520px] border-t-10 border-blue-500">
  <div class="pl-4 pt-2 pb-2">
    <h1 class="text-xl font-semibold">TAGIHAN KONSOLIDATOR</h1>
  </div>
  ...
  <div class="bg-blue-500 text-white rounded-t-md py-2 px-4">
    <div class="flex items-center">
      <Icon fa="arrow-left" class="cursor-pointer mr-2 font-bold hover:text-yellow-500" title="Kembali"
        @click="onBack" />
      <div>
        <h1 class="text-20px font-bold">Tagihan Konsolidator</h1>
        <p class="text-gray-100">Transaksi Tagihan Konsolidator</p>
      </div>
    </div>
  </div>
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <div class="flex space-x-2">
      ...
      <FieldNumber :bind="{ readonly: !actionText  }" class=" !mt-3 w-full" :value="values.ppn"
        ...
        placeholder="Masukkan Persentase PPN" :check="false" />

    </div>
    <div class="flex space-x-2">
      <span class="text-lg font-semibold items-center !mt-3">%</span>
      <FieldNumber :bind="{ readonly: !actionText  }" class=" !mt-3 w-full" :value="values.persentase_konsolidator_kont"
        :errorText="formErrors.persentase_konsolidator_kont?'failed':''" @input="v=>values.persentase_konsolidator_kont=v" 
        :hints="formErrors.persentase_konsolidator_kont" label="persentase konsolidator kontainer"
        placeholder="Masukkan Persentase Jasa Tia" :check="false" />
    ...
    <div class="hidden">
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="values.tipe_tagihan || 'KONSOLIDATOR'"
        :errorText="formErrors.tipe_tagihan?'failed':''" @input="v=>values.tipe_tagihan=v" :hints="formErrors.tipe_tagihan"
        label="Tipe tagihan" placeholder="Tipe tagihan" :check="false" type="textarea" />
    </div>
    ...
    <table class="w-full bg-white rounded-lg shadow-md overflow-hidden">
      <thead class="bg-gray-100">
        <tr>
          ...
          <th class="p-3 text-left text-sm font-semibold text-gray-700 w-1/2">Jumlah Konsolidator</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="border-b border-gray-200 hover:bg-gray-50">
          ...
          <td class="p-3 border-r text-sm text-gray-900">{{ formatCurrency(values.total_lain2_ppn) || 0 }}</td>
        </tr>
        <tr class="border-b border-gray-200 hover:bg-gray-50">
          <td class="p-3 border-r text-sm border-r text-gray-700">Total PPN</td>
          <td class="p-3 border-r text-sm text-gray-900">{{ formatCurrency(values.total_ppn) || 0 }}</td>
        </tr>
        <tr class="border-b border-gray-200 hover:bg-gray-50">
          <td class="p-3 border-r text-sm border-r text-gray-700">Total Jasa Tia (ANGK)</td>
          <td class="p-3 text-sm text-gray-900">{{ formatCurrency(values.total_jasa_angkutan) || 0 }}</td>
        </tr>
        <tr class="border-b border-gray-200 hover:bg-gray-50">
          <td class="p-3 border-r text-sm border-r text-gray-700">Total Lain-lain (NON PPN)</td>
          <td class="p-3 border-r text-sm text-gray-900">{{ formatCurrency(values.total_lain_non_ppn) || 0 }}</td>
        </tr>
        <tr class="hover:bg-gray-50 border-b">
          <td class="p-3 border-r text-sm border-r text-gray-700">Tarif DP</td>
          <td class="p-3 border-r text-sm text-gray-900">{{ formatCurrency(values.total_tarif_dp) || 0 }}</td>
        </tr>
        <tr class=" border-gray-200 hover:bg-gray-50">
          <td class="p-3 border-r text-sm border-r text-gray-700">Grand Total</td>
        ...
        <table class="w-full bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <tbody>
            <tr v-for="(item, i) in detailArr1" :key="item.id" class="border-b border-gray-200 hover:bg-gray-50"
              ...
              <!-- Persentase Jasa with FieldNumber -->
              <td class="p-3 text-sm text-gray-900 border-r">
