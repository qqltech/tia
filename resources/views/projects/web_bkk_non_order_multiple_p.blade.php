...
$dataBkkNonOrder = \DB::table('t_bkk_non_order as tb') 
    ->select('tb.*', 'tb.total_amt')
    ->whereIn('tb.id', $ids)
    ->orderByRaw("array_position(ARRAY[" . implode(',', $ids) . "]::bigint[], tb.id::bigint)")
    ->get();

$tipeBkk = $dataBkkNonOrder->first()->tipe_bkk;

$noUrut = \DB::table('t_bkk')
            ->where('status', 'PRINTED') 
            ->count();

// FIX NO URUT DAN TIPE BKK NYA, SAAT INI MASIH HARDCODE PPJK
...
$detailRaw = \DB::table('t_bkk_non_order_d as tbd') 
    ->leftJoin('m_coa as mc', 'mc.id', 'tbd.m_coa_id')
    ->whereIn('tbd.t_bkk_non_order_id', $ids) 
    ->select('tbd.*', 'mc.nomor as nomor_coa', 'mc.nama_coa', 'tbd.t_bkk_non_order_id') 
    ->orderBy('tbd.t_bkk_non_order_id')
    ->orderBy('tbd.id')
    ->get();
...
foreach ($detailRaw as $d) {
    ...
    $renderRows[] = [
        ...
        // Kolom 'orders' dikosongkan karena ini Non Order
        'orders'      => [], 
        'nominal_sum' => floatval($d->nominal),
        'bkk_ids'     => [$d->t_bkk_non_order_id] 
    ];
}
...
$joined_no_bkk = implode(', ', $dataBkkNonOrder->pluck('no_bkk')->toArray()); // Menggunakan $dataBkkNonOrder
// $first_tanggal = $dataBkkNonOrder->first()->tanggal ?? date('Y-m-d'); // Menggunakan $dataBkkNonOrder
...
<body class="continuous_form">
  <!-- <pre>{{var_dump($dataBkkNonOrder->first()->tipe_bkk)}}</pre> -->
    <section class="sheet padding-3mm">
        ...
        <table cellspacing="0" cellpadding="3" style="width:100%; border: 0.5px solid black; border-collapse: collapse; font-size: 9px;">
            <tr>
                <th style="width: 15%; text-align: center;">No. Referensi</th>
            ...
            @foreach($renderRows as $row)
                <tr>
                    <td style="text-align: center; padding-left:6px;">
                        {{ $dataBkkNonOrder->where('id', $row['bkk_ids'][0])->first()->no_reference ?? '-' }}
                    </td>
        ...
        <table style="width: 100%; border: 0.5px solid black; margin-top:6px; border-collapse: collapse;">
            ...
            <tr>
                ...
                <td style="text-align: center; padding-top: 12%;">( <span>{!! $dataBkkNonOrder->first()?->nama_penerima ?? str_repeat('&nbsp;', 50) !!}</span> )</td>
            </tr>
        </table>
