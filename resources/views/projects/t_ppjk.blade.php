...
<div class="bg-white rounded-md min-h-[520px] border-t-10 border-blue-500">
    ...
    <!-- TABLE -->
    <TableApi ref='apiTable' :api="table.api" :columns="table.columns" :actions="table.actions" class="max-h-[450px] pt-2 !px-4 !pb-8">
        <template #header>
...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
    ...
    <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 items-start">
        <div class="relative">
          ...
          <FieldX :bind="{ readonly: true || !actionText }" class="pt-1" :value="data.no_draft" :errorText="formErrors.no_draft?'failed':''" @input="v=>data.no_draft=v" :hints="formErrors.no_draft" label="No. Draft" placeholder='Auto Generate by System' :check="false" />
        </div>
        ...
        <FieldPopup
          ...
          :value="data.no_ppjk_id" @input="v=>data.no_ppjk_id=v"
          :errorText="formErrors.no_ppjk_id?'failed':''" 
          :hints="formErrors.no_ppjk_id"
          valueField="id" displayField="no_aju"
          :api="{
          ...
          placeholder="No PPJK" :check="false"
          :columns="[{
            headerName: 'No',
        ...
        <FieldPopup
          ...
          placeholder="No Buku Order" :check="false" 
          :columns="[{
            headerName: 'No',
            valueGetter:(p)=>p.node.rowIndex + 1,
            width: 60,
          ...
          @update:valueFull="(res) => {
            if(res === null){
              ...
              data.kode_customer = res['m_customer.kode'];
              getNpwpCustomer(res['m_customer.id']);
            }
          }"
        />
        ...
        <FieldPopup
          :bind="{ readonly: !actionText }"
          :value="data.m_customer_id" @input="(v)=>data.m_customer_id=v"
        ...
        <FieldX :bind="{ readonly: true || !actionText }" class="pt-1" :value="data.kode_customer" :errorText="formErrors.kode_customer?'failed':''" 
          @input="v=>data.kode_customer=v" :hints="formErrors.kode_customer" placeholder="Kode Customer" :check="false" />

        <FieldX :bind="{ readonly: true || !actionText }" class="pt-1" :value="data.no_npwp" :errorText="formErrors.no_npwp?'failed':''" 
          @input="v=>data.no_npwp=v" :hints="formErrors.no_npwp" placeholder="NPWP" :check="false" />
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.no_peb_pib" :errorText="formErrors.no_peb_pib?'failed':''" 
          @input="v=>data.no_peb_pib=v" :hints="formErrors.no_peb_pib" placeholder="No PEB / PIB" :check="false" type="number" />
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.tanggal_peb_pib" :errorText="formErrors.tanggal_peb_pib?'failed':''" 
          @input="v=>data.tanggal_peb_pib=v" :hints="formErrors.tanggal_peb_pib" placeholder="Tanggal PEB / PIB" :check="false" 
          type="date" />
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.no_sppb" :errorText="formErrors.no_sppb?'failed':''" 
          @input="v=>data.no_sppb=v" :hints="formErrors.no_sppb" placeholder="No SPPB" :check="false" type="number" />
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.tanggal_sppb" :errorText="formErrors.tanggal_sppb?'failed':''" 
          @input="v=>data.tanggal_sppb=v" :hints="formErrors.tanggal_sppb" placeholder="Tanggal SPPB" :check="false" 
          type="date" />
        <FieldNumber class="pt-1"
          :bind="{ readonly: !actionText }" :value="data.invoice" @input="(v)=>data.invoice=v"
          :errorText="formErrors.invoice?'failed':''" :hints="formErrors.invoice" placeholder="Invoice" :check="false" />
        <FieldNumber class="pt-1"
          :bind="{ readonly: !actionText }" :value="data.ppn_pib" @input="(v)=>data.ppn_pib=v"
          :errorText="formErrors.ppn_pib?'failed':''" :hints="formErrors.ppn_pib" placeholder="PPN PIB" :check="false" />
        <FieldX :bind="{ readonly: true, disabled:true }" class="pt-1" :value="data.currency" :errorText="formErrors.currency?'failed':''" 
          @input="v=>data.currency=v" :hints="formErrors.currency" placeholder="Currency" :check="false" 
        />
        <FieldNumber
        class="pt-1"
          :bind="{ readonly: true, disabled:true }"
          :value="data.nilai_kurs" @input="(v)=>data.nilai_kurs=v"
          :errorText="formErrors.nilai_kurs?'failed':''" 
          :hints="formErrors.nilai_kurs"
        ...
        -->
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.catatan" :errorText="formErrors.catatan?'failed':''" 
          @input="v=>data.catatan=v" :hints="formErrors.catatan" placeholder="Catatan" :check="false" type="textarea" />
