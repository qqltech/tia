...
<div class="bg-white rounded-md min-h-[520px] border-t-10 border-blue-500">
  <div class="pl-4 pt-2 pb-2">
    <h1 class="text-xl font-semibold">BUKU PENYESUAIAN</h1>
  </div>
...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
  <div class="bg-blue-500 text-white rounded-t-md pt-2 pb-3 px-4">
    <div class="flex items-center gap-2">
      ...
      <div class="flex flex-col py-1 gap-1">
        <h1 class="text-lg font-bold leading-none">Form Buku Penyesuaian</h1>
        <p class="text-gray-100 leading-none">Transaction Buku Penyesuaian</p>
      </div>
    </div>
  </div>
  <!-- HEADER -->
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.no_buku_penyesuaian"
        :errorText="formErrors.no_buku_penyesuaian?'failed':''" @input="v=>data.no_buku_penyesuaian=v"
        :hints="formErrors.no_buku_penyesuaian" label="No. Buku Penyesuaian" placeholder="No. Buku Penyesuaian"
        :check="false" />
    </div>
    <div>
      <FieldX :bind="{ readonly: true, disabled: true , required: true}" class="w-1/2 !mt-3"
        :value="data.tanggal_buku_penyesuaian" :errorText="formErrors.tanggal_buku_penyesuaian?'failed':''"
        @input="v=>data.tanggal_buku_penyesuaian=v" :hints="formErrors.tanggal_buku_penyesuaian" :check="false"
        type="date" label="Tanggal Penyesuaian" placeholder="Pilih Tanggal Penyesuaian" />
    </div>
    <!-- <div>
      <FieldX :bind="{ readonly: true }" class="w-1/2 !mt-3" :value="data.tanggal_buku_penyesuaian"
        :errorText="formErrors.tanggal_buku_penyesuaian?'failed':''" @input="v=>data.tanggal_buku_penyesuaian=v" :hints="formErrors.tanggal_buku_penyesuaian"
        :check="false" type="date" label="Tgl Buku Penyesuaian" placeholder="Pilih Tgl Buku Penyesuaian" />
    </div> -->
    ...
    <div>
      ...
      }" displayField="no_bkk" valueField="id" :bind="{ readonly: !actionText }" :value="data.no_bkk_id"
        @input="(v)=>data.no_bkk_id=v" @update:valueFull="(res)=>{
          // getTarifPremi(res);
        if(res){
      ...
      }" :errorText="formErrors.no_bkk_id?'failed':''" class="w-full !mt-3" :hints="formErrors.no_bkk_id"
        label="No. BKK" placeholder="Pilih No. BKK" :check='false' :columns="[
        {
          headerName: 'No',
          valueGetter:(p)=>p.node.rowIndex + 1,
          width: 60,
    ...
    <!-- <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.tanggal" @input="v=>data.tanggal=v"
        :errorText="formErrors.tanggal?'failed':''" :hints="formErrors.tanggal" placeholder="Tanggal BKK" label="Tanggal BKK" :check="false" />
    </div> -->

    <div>
      <FieldX :bind="{ readonly: true, disabled: true , required: true}" class="w-1/2 !mt-3" :value="data.tanggal"
        :errorText="formErrors.tanggal?'failed':''" @input="v=>data.tanggal=v" :hints="formErrors.tanggal"
        :check="false" type="date" label="Tanggal BKK" placeholder="Pilih Tanggal BKK" />
    </div>
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-1/2 !mt-3" :value="data.status"
  ...
  <div class="w-full !mt-2 !mb-2 !ml-3">
    ...
    <p type="number" class="w-full !mt-3" style="font-size: 20px; font-weight: bold;">Total Penyesuaian : Rp. {{
      data.total_amt_tampil }}</p>
  </div>
