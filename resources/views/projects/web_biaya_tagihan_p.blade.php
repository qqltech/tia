...
// Decode user: accept numeric user OR base64-encoded numeric user (tetap ada jika dibutuhkan)
...
<style>
/* Latar halaman -- beri kontras supaya "lembar" putih terlihat seperti pada gambar 2 */
html, body {
    margin: 0;
    padding: 0;
    background: #efefef; /* abu-abu muda seluruh halaman */
    font-family: Arial, sans-serif;
    -webkit-font-smoothing: antialiased;
}

/* Wrapper untuk memberi ruang atas/bawah di halaman */
.page-wrapper {
    padding: 24px 24px; /* ruang dari tepi browser agar ada area abu-abu di sekeliling */
    box-sizing: border-box;
}

/* Lembar/kertas putih dengan border di tengah */
.print-sheet {
    width: 800px;                 /* sesuaikan lebar kertas */
    max-width: 95%;               /* responsif */
    margin: 0 auto 28px;          /* tambahkan margin-bottom untuk spasi antar sheet di layar */
    background: #ffffff;          /* putih murni di belakang border */
    border: 1px solid #000;       /* border hitam seperti contoh */
    padding: 18px;                /* ruang isi dalam border */
    box-sizing: border-box;
    box-shadow: 0 6px 18px rgba(0,0,0,0.18); /* bayangan lembut seperti kertas di atas latar */
    color: #000;
    font-size: 10px;

    /* Pastikan tiap sheet tidak terpotong di tengah */
    page-break-inside: avoid;
    break-inside: avoid;

    /* Paksa pemisahan halaman saat print/print-preview */
    page-break-after: always;
    break-after: page;
}

/* jangan tambahkan page-break setelah sheet terakhir */
.print-sheet:last-child {
    page-break-after: auto;
    break-after: auto;
    margin-bottom: 0; /* hilangkan spasi ekstra setelah terakhir */
}

/* Pastikan semua tabel di dalam lembar tidak memiliki background selain putih lembarnya */
.print-sheet table {
    background: transparent;
}

/* Jika ada thead berwarna, override agar tetap terlihat natural di lembar putih */
.print-sheet thead tr {
    background: transparent;
}

/* Styling khusus saat print:
   - Pastikan lembar tetap putih
   - Hilangkan shadow (tidak diperlukan pada cetak)
   - Biarkan border agar tercetak
*/
@media print {
    html, body {
    ...
    .print-sheet {
        ...
        margin: 0 0 12mm 0; /* sisipkan jarak antar halaman di cetak (12mm contoh) */
        width: auto;
        max-width: 100%;
        border: 1px solid #000;
        /* ulang agar lebih kompatibel dengan engine print */
        page-break-inside: avoid;
        page-break-after: always;
        break-after: page;
    }
...
@foreach($ids as $id)
    ...
    $bukuOrder = \DB::table('t_buku_order')->where('id', $id)->first();
    if (!$bukuOrder) {
    ...
    if (!$dataBukuOrder) {
        echo "<div class='print-sheet'><p>Data Buku Order tidak ditemukan untuk id {$id}.</p></div>";
        continue;
    }
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 10px; line-height: 1.2; font-weight:bold;">
        <tbody>
          <tr>
            ...
            <td colspan="2" style="padding: 2px;">{{$dl2->no_aju ?? '-'}}</td>
          ...
          <tr>
            ...
            <td ></td>
            <td style="padding: 2px; text-align: left; width: 57px;">Tgl. Order</td>
            <td style="padding: 2px; width: 10px;">:</td>
          ...
          <tr>
            ...
            <td style="padding: 2px; width:8px;">:</td>
          ...
          <tr>
            ...
            <td style="padding: 2px; width:10px;">:</td>
            <td style="padding: 2px; width:200px;">{{$dl->petugas_pengkont_nama ?? '-'}}</td>
            <td></td>
            <td style="padding: 2px; text-align: left; width:55.5px;">Tgl.</td>
            <td style="padding: 2px; width:10px;">:</td>
            <td style="padding: 2px; text-align: left; white-space: nowrap; width: 58px;">{{formatDate($n->tanggal_pengkont)}}</td>
          </tr>

          <tr>
            <td style="padding: 2px; width: 90px;">Pemasukan</td>
            <td style="padding: 2px; width: 10px;">:</td>
            <td style="padding: 2px; width:200px;">{{$dl->petugas_pemasukan_nama ?? '-'}}</td>
            <td></td>
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 10px; line-height: 1.2; font-weight:bold;">
        <tbody>
          <tr>
            ...
            <td style="padding: 3px; width:8px;">:</td>
          ...
          <tr>
            <td style="padding: 3px;">9. Lain-lain</td>
            <td style="padding: 3px width:10px;">:</td>
