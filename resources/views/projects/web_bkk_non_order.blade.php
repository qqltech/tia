...
$dataBKKNonOrder = \DB::select(' select
*
from t_bkk_non_order as tbno
where tbno.id = ?
',[$req['id']]);

$detailBKKNonOrder = \DB::select(' select
*, mc.nomor as nomor_coa, mc.nama_coa as nama_coa
from t_bkk_non_order_d as tbnod
left join m_coa as mc on mc.id = tbnod.m_coa_id
where tbnod.t_bkk_non_order_id = ?
', [$req['id']]);

$n = $dataBKKNonOrder[0];
$numbering = 1;
$total_nominal=0;
foreach($detailBKKNonOrder as $dbno){
$total_nominal+=floatval($dbno->nominal);
}
...
$temp = ' sebelas';
  ...
  . ' juta' . terbilang($number % 1000000); } return $temp; } @endphp <head>
  <meta charset="utf-8" />
  ...
  <body class="continuous_form">
    <section class="sheet padding-3mm" style="padding-right: 30px;">
      <table style="width: 100%; font-size: 12px;">
        <tr>
          <td style="text-align: center; font-family: sans-serif; font-weight: bold;">BKK NON ORDER</td>
        </tr>
      </table>
      <!-- Tabel 1 -->
      <!-- {{var_dump($user)}} -->
      <table style="width:100%; border: 0.5px solid black; line-height: 1.5;">
        <tr>
          <td style="width: 60%;">No. Bukti
            ...
            <span style="margin-left: 5%; font-weight: bold; text-decoration: underline;">{{$n->no_bkk}}</span>
          </td>
          ...
          <td colspan="3"
            ...
            <table cellspacing="0" cellpadding="3" border="0.5"
              ...
              <tr>
                ...
                <th style="width: 20%; text-align: center; border: 0.5px solid black;">Nomor COA</th>
                <th style="width: 25%; text-align: center; border: 0.5px solid black;">Nama COA</th>
                <th style="width: 22%; text-align: center; border: 0.5px solid black;">Detail</th>
              ...
              <tr>
                ...
                <td style="width: 20%; text-align: center; border: 0.5px solid black;">{{$dbno->nomor_coa??'-'}}</td>
                <td style="width: 25%; text-align: center; border: 0.5px solid black;">{{$dbno->nama_coa??'-'}}</td>
                @if(!$dbno->keterangan)
                <td style="width: 22%; text-align: center; border: 0.5px solid black;">
                  -</td>
              </tr>
              @elseif($dbno->keterangan)
              <td style="width: 22%; text-align: left; border: 0.5px solid black;">
                {{$dbno->keterangan}}</td>
              @endif
              <td style="width: 28%; text-align: center; border: 0.5px solid black;">Rp {{number_format($dbno->nominal,2,',','.')}}</td>
        </tr>
        @endforeach
      </table>
      <!-- Tabel 3 -->
      <table style="width: 100%; border: 0.5px solid black; border-top: none;">
        <tr>
          ...
          <td style="width: 32%; padding-left: 2%; text-align: center; font-weight: bold;">Rp
            {{number_format($total_nominal,2,',','.')}}</td>
      ...
      <tr style="width: 100%; font-size: 10px;">
        <td style="width:100%; font-weight: bold; border-bottom: 0.5px solid black; white-space: nowrap;">Dicetak pada
          tgl : {{$currentDate}} &nbsp; jam {{$currentTime}} &nbsp; Operator : {{$userValue->name}} #
          {{$userValue->username}}</td>
