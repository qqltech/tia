...
<div class="bg-white p-1 rounded-md min-h-[520px] border-t-10 border-blue-500">
  ...
  <div class="flex justify-between items-center px-2.5 py-1">
    ...
    <div class="flex items-center gap-x-2">
      ...
      <div class="flex gap-x-2">
        ...
        <!-- <div class="flex my-auto h-4 w-px bg-[#6E91D1]"></div>
        <button @click="filterShowData('TUTUP BUKU')" :class="filterButton === 'TUTUP BUKU' ? 'bg-sky-600 text-white hover:bg-sky-600' 
  ...
  <TableApi ref='apiTable' :api="landing.api" :columns="landing.columns" :actions="landing.actions"
    ...
    <template #header>
      ...
      <div>
        <ButtonMultiSelect title="Preview Biaya" @add="onPreviewCover" :api="{
          url: `${store.server.url_backend}/operation/${endpointApi}`,
          headers: {'Content-Type': 'Application/json', authorization: `${store.user.token_type} ${store.user.token}`},
          params:{
            searchfield: 'this.no_buku_order, m_customer.kode, m_customer.nama_perusahaan, this.tgl, this.voyage, this.status',
      ...
      <!-- <div>
        <button class="border border-blue-600 text-blue-600 bg-white  hover:bg-blue-600 hover:text-white duration-300 transform hover:-translate-y-0.5 rounded-md py-1 px-2"
        @click="handleTutupBukuButton">
          kliken su
        </button>
      </div> -->
    </template>
