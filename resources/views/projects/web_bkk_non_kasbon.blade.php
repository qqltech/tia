...
foreach ($detailBkkNonKasbon as $dbnk) {
    $total_nominal += floatval($dbnk->nominal);
}
...
<body class="continuous_form">
  <section class="sheet padding-3mm" style="padding-right: 30px;">
    <table style="width: 100%; font-size: 12px;">
        <tr>
          <td style="text-align: center; font-family: sans-serif; font-weight: bold; padding: 2%;">BKK NON KASBON</td>
        </tr>
      </table>
<!-- Tabel 1 -->
        ...
        <td colspan="3"
          ...
          <table cellspacing="0" cellpadding="3" border="0.5"
            ...
            <tr>
              ...
              <td style="width: 20%; text-align: center; border: 0.5px solid black;">{{$dbnk->nomor_coa??'-'}}</td>
              <td style="width: 25%; text-align: center; border: 0.5px solid black;">{{$dbnk->nama_coa??'-'}}</td>
              @if(!$dbnk->keterangan)
              <td style="width: 22%; text-align: center; border: 0.5px solid black;">
                -</td>
              @elseif($dbnk->keterangan)
             <td style="width: 22%; text-align: center; border: 0.5px solid black;">
                {{$dbnk->keterangan}}</td>
              @endif
              <td style="width: 28%; text-align: center; border: 0.5px solid black;">Rp {{number_format($dbnk->nominal,2,',','.')??'-'}}</td>
            </tr>
            @endforeach
          </table>
 <!-- Tabel 3 -->
          <table style="width: 100%; border: 0.5px solid black; border-top: none;">
            <tr>
              ...
              <td style="width: 32%; padding-left: 2%; text-align: center; font-weight: bold;">Rp {{number_format($n->total_amt,2,',','.')}}</td>
