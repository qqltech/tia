...
@verbatim
  <div class="flex flex-col border rounded-md shadow-md md:w-full w-full p-0 bg-white border-none">
    ...
    <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
      ...
      <div>
        <FieldX :bind="{ readonly: true }" class="w-full !mt-3"
          :value="values.no_angkutan" :errorText="formErrors.no_angkutan?'failed':''"
          @input="v=>values.no_angkutan=v" :hints="formErrors.no_angkutan" 
          label="No. Angkutan"
          placeholder="No. Angkutan"
          :check="false"
        />
      </div>
      <div>
      ...
      <div>
        <FieldX :bind="{ readonly: true }" class="w-full !mt-3"
          :value="values.party" :errorText="formErrors.party?'failed':''"
          @input="v=>values.party=v" :hints="formErrors.party" 
          :check="false"
          label="Party"
          placeholder="Party"
        />
          ...
          <tbody>
            <tr v-for="(item, i) in detailArr" :key="item.id" class="border-t" v-if="detailArr.length">
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: true , required: true}" class="w-full !mt-3"
                    :value="item.no_spk" :errorText="formErrors.no_spk?'failed':''"
                    @input="v=>item.no_spk=v" :hints="formErrors.no_spk" 
                    :check="false"
                    placeholder=""
                />
              </td>
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true) }" class="w-full !mt-3" :value="item.no_container"
                  :errorText="formErrors.no_container?'failed':''" @input="v=>item.no_container=v" :hints="formErrors.no_container"
                  :check="false" label="" placeholder="" />
              </td>
              <td class="p-2 border border-[#CACACA]" style="min-width: 150px !important;">
                <FieldSelect
                  :bind="{ disabled: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true), clearable:true }" class="w-full !mt-3"
              ...
              <td class="p-2 border border-[#CACACA]" style="min-width: 100px !important;">
                <FieldX :bind="{ readonly: true, disabled: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true), required: true}" class="w-full !mt-3"
                    :value="item.tanggal_out" :errorText="formErrors.tanggal_out?'failed':''"
                    @input="v=>updateTanggalOut(v, item)" :hints="formErrors.tanggal_out" 
                    :check="false"
                    type="date"
                    placeholder=""
                />
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: !actionText , required: false}" class="w-full !mt-3"
                    :value="item.jam_out" :errorText="formErrors.jam_out?'failed':''"
                    @input="v=>updateJamOut(v, item)" :hints="formErrors.jam_out" 
                    :check="false"
                    type="time"
                    placeholder=""
                />
              ...
              <td class="p-2 border border-[#CACACA]" style="min-width: 100px !important;">
                <FieldX :bind="{ readonly: true, disabled: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true), required: false, clearable:false}" class="w-full !mt-3"
                    :value="item.tanggal_in" :errorText="formErrors.tanggal_in?'failed':''"
                    @input="v=>updateTanggalIn(v, item)" :hints="formErrors.tanggal_in" 
                    :check="false"
                    type="date"
                    placeholder="" fa-icon=""
                />
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: !actionText , required: false}" class="w-full !mt-3"
                    :value="item.jam_in" :errorText="formErrors.jam_in?'failed':''"
                    @input="v=>updateJamIn(v, item)" :hints="formErrors.jam_in" 
                    :check="false"
                    type="time"
                    placeholder=""
                />
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="item.free"
                  :errorText="formErrors.free?'failed':''" @input="v=>item.free=v" :hints="formErrors.free"
                  :check="false" label="" placeholder="" />
              </td>
              <td class="p-2 border border-[#CACACA]">
                <FieldNumber :bind="{ readonly: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true) }" class="w-full !mt-3" :value="item.biaya_lain_lain"
                  :errorText="formErrors.biaya_lain_lain?'failed':''" @input="v=>item.biaya_lain_lain=v" :hints="formErrors.biaya_lain_lain"
                  :check="false" label="" placeholder="" />
              </td>
              <td class="p-2 border border-[#CACACA]" style="min-width: 150px !important;">
                <FieldSelect
                  :bind="{ disabled: item.item_no_spk === null ? false : (item.no_spk ? !item.no_spk.includes('Luar') : true), clearable:true }" class="w-full !mt-3"
                  :value="item.nama_angkutan_id"  @input="v=>{
                    if(v){
                  ...
                  :errorText="formErrors.nama_angkutan_id?'failed':''" 
                  :hints="formErrors.nama_angkutan_id"
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="item.tarif_los_cargo"
                  :errorText="formErrors.tarif_los_cargo?'failed':''" @input="v=>item.tarif_los_cargo=v" :hints="formErrors.tarif_los_cargo"
                  :check="false" label="" placeholder="" />
              </td>
              <td class="p-2 border border-[#CACACA]" style="min-width: 100px !important;">
                <FieldX :bind="{ readonly: true,disabled: true, required: true}" class="w-full !mt-3"
                    ...
                    @input="v=>item.tgl_stuffing=v" :hints="formErrors.tgl_stuffing" 
                    :check="false"
                    type="date"
                    placeholder=""
                />
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: item.item_no_spk === null ? true : (item.no_spk ? item.no_spk.includes('SPK') : true) }" class="w-full !mt-3" :value="item.angkutan_pelabuhan"
                  :errorText="formErrors.angkutan_pelabuhan?'failed':''" @input="v=>item.angkutan_pelabuhan=v" :hints="formErrors.angkutan_pelabuhan"
                  :check="false" label="" placeholder="" />
              </td>
              <td class="p-2 border border-[#CACACA]">
              ...
              <td class="p-2 border border-[#CACACA]">
                <FieldX :bind="{ readonly: item.item_no_spk === null ? true : (item.no_spk ? item.no_spk.includes('SPK') : true) }" type="textarea" class="w-full !mt-3" :value="item.catatan"
