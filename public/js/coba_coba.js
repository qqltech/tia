...
import { ref, readonly, computed, reactive, inject, onMounted, onBeforeMount, onBeforeUnmount, watchEffect, onActivated } from 'vue'
...
function acceptType(nameField) {
  ...
  if (actionText.value && file) {
    const indexFile = file.lastIndexOf('.')
    const extensionFile = file.slice(indexFile + 1).toLowerCase()
    const imageExtensions = ['jpeg', 'jpg', 'png', 'gif', 'bmp', 'webp', 'svg', 'tiff', 'ico', 'heic', 'heif', 'jfif', 'pjpeg', 'pjp', 'avif']
    const isImage = imageExtensions.includes(extensionFile)
    return isImage ? 'image/*' : 'application/pdf, application/msword'
  }
  return 'image/*, application/pdf, application/msword'
}
// Table Detail
const addDetail = () => {
