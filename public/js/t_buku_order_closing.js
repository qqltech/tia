...
// ------------------------------ PERSIAPAN
const endpointApi = 't_buku_order_closing'
onBeforeMount(() => {
  document.title = 'Buku Order Closing'
})
...
const monthNames = ref(['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'])
const optYear = ref([])
const defaultValues = async () => {
  ...
  isOpen.value = true
  values.grup = 'TAHUNAN'
  values.periode_tahun = new Date().getFullYear()
...
function autoGenerate() {
  ...
  swal.fire({
    icon: 'question',
    text: 'Tampilkan data Outstanding tahun ini?',
    showDenyButton: true,
    ...
    if (res.isConfirmed) {
      try {
        ...
        const dataURL = `${store.server.url_backend}/operation/t_buku_order_closing/get_outstanding`;
        ...
        if (resultJson.data) {
          ...
          values.alasan_closing = resultJson.data?.alasan_closing;
          values.is_closed = resultJson.data?.is_closed;

          console.log('halo :', values.alasan_closing)
          console.log('aloha :', resultJson.data)
        ...
        const totalTransaksi = detailArr.value.reduce(
          (total, item) => total + (item.details?.length || 0),
          0
        );
        ...
        if (totalTransaksi === 0) {
          swal.fire({
            icon: 'success',
            text: 'Semua transaksi sudah clear! Anda dapat melakukan Closing.'
          });
          activeTab.value = 'outstanding';
        } else {
          swal.fire({
            icon: 'warning',
            text: `Terdapat ${totalTransaksi} transaksi buku order yang belum final!`
          });
          activeTab.value = 'outstanding';
        }
...
async function onSave(prms) {
  try {
    ...
    if (!hasGenerated.value) {
      swal.fire({
        icon: 'error',
        text: `Silakan klik tombol "Show Data" terlebih dahulu untuk mengecek Outstanding!`
      });
      return;
    }
    ...
    await fetch(`${store.server.url_backend}/operation/t_buku_order_closing/closing`, {
      method: 'POST',
      headers: {
        'Content-Type': 'Application/json',
        Authorization: `${store.user.token_type} ${store.user.token}`
    ...
    swal.fire({
      icon: 'success',
      text: 'Closing Tahunan Berhasil Disimpan!'
    });
    onReset(false)
    formErrors.value = {}
