...
// ENDPOINT API
const endpointApi = 't_premi_batal'
onBeforeMount(() => {
  document.title = 'Transaction Premi Batal'
})
...
const table = reactive({
  api: {
    ...
    params: {
      simplest: false,
      searchfield: 'this.id, this.no_premi_batal, this.tgl, t_spk_angkutan.total_sangu, this.catatan',
    },
    onsuccess(response) {
  ...
  columns: [
    ...
    },
    {
      ...
      field: 't_spk_angkutan.total_sangu',
      flex: 1,
...
onBeforeMount(async () => {
  if (actionText.value === 'Create' || data.status === 'DRAFT') {
    ...
    data.tgl_premi = getCurrentDateFormatted();
  }
  ...
  try {
    ...
    await fetchData(dataURL, { join: true, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_premi_batal_d);
    ...
    await fetchData(dataURLSPK, { join: true, transform: false }).then((res) => {
      ...
      if (res.data) {
        ...
        data.waktu_out = res.data['waktu_out.deskripsi'];
        data.no_bon_sementara = res.data.no_bon_sementara;
        data.tanggal_bon = res.data.tanggal_bon;
        data.head_deskripsi = res.data['head.deskripsi'];
        data.tanggal_in = res.data.tanggal_in;
        data.waktu_in = res.data['waktu_in.deskripsi'];
