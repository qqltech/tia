...
// ------------------------------ PERSIAPAN
const endpointApi = '/t_dp_penjualan'
onBeforeMount(() => {
  document.title = 'Down Payment Penjualan'
})
...
const landing = reactive({
  actions: [
    ...
    },
    {
      ...
      async click(row) {
        ...
        if (confirmResult.isConfirmed) {
          try {
            const dataURL = `${store.server.url_backend}/operation/t_dp_penjualan/post?id=${row.id}`;
            isRequesting.value = true;
    ...
    //   async click(row) {
    //     try {
    //       const dataURL = `${store.server.url_backend}/operation/t_dp_penjualan/print?id=${row.id}`;
    //       isRequesting.value = true;
    //       const response = await fetch(dataURL, {
    //         method: 'POST',
    //         headers: {
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield: 'this.id, this.no_draft, this.no_dp, m_customer.nama_perusahaan, t_buku_order.no_buku_order, this.tgl_dp, t_buku_order.jenis_barang,tipe_dp.deskripsi, this.status',
    },
    onsuccess(response) {
