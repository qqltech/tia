...
// ------------------------------ PERSIAPAN
const endpointApi = '/t_credit_note'
onBeforeMount(() => {
  document.title = is_approval ? 'Approval Credit Note' : 'Transaksi Credit Note'
})
  ...
  if (isRead) {
    ...
    try {
      ...
      if (route.query.is_approval) {
        dataURLAprv = `${store.server.url_backend}/operation/t_credit_note/detail?id=${route.params.id}`
        ...
        initialValues.tipe_cn = initialValues['tipe_credit_note.deskripsi4']
         detailArr.value = initialValues.t_credit_note_d.map((dt) => ({
          ...dt,
        ...
        const allSubCreditNotes = initialValues.t_credit_note_d.flatMap(item => item.t_sub_credit_note);

        subDetail.value = allSubCreditNotes.map((dt2) => ({
          ...dt2,
        ...
        if (actionText.value?.toLowerCase() === 'copy') {
          ...
          initialValues.no_credit_note = null;
        }
...
const subDetail = computed(()=>{
  let data = [];
  detailArr.value.forEach(dt => {
    dt.t_sub_credit_note.forEach(det => {
      data.push(det);
    })
  });
  return data;
...
const totalCreditNote = computed(() => {
  let total = 0;
  detailArr.value.forEach(dt => {
...
const subTotal2 = (item) => {
  const total = item.t_sub_credit_note.reduce((acc, dt) => acc + (dt.amount ?? 0), 0);
...
//     let total = 0;
//     const cn_d_id = dt['id'];
//     subDetail.value.forEach(dt2 => {
//       if (cn_d_id === dt2['t_tagihan_id'] || cn_d_id === dt2['t_purchase_invoice_id'] ) {
//         total = total + (dt2['amount'] ?? 0);
//       }
//     })
//     dt.sub_total_amount = total;
//     return total;
//   });
// });
...
const removeDetail = async (index) => {
  ...
  // Remove items from detailArr2 where t_credit_note_d_id matches the removed item's 
  subDetail.value = subDetail.value.filter(item => item.t_credit_note_d_id !== removedItemId);
}
...
//   // if (!values.no_tagihan) {
//   //   swal.fire({
//   //     icon: 'error',
//   //     text: 'No Tagihan harus dipilih!'
//   //   });
//   // return;
//   // }
//   e.forEach((row, index) => {
//     row.no_urut = detailArr2.value.length + 1;
//     row.t_credit_note_d_id = values.no_tagihan;
//     row.m_coa_id = row.id
//     row.nama_coa = row.id
//     row.tipe_perkiraan = row['tipe_perkiraan.id']
//     row.amount = 0
...
async function onSave(status) {
  try {
    // values.t_credit_note_d = detailArr;
    // values.t_sub_credit_note = detailArr2;

    values.t_credit_note_d = detailArr.value.map(item => ({
      ...item,
      t_sub_credit_note: item.t_sub_credit_note.map((dt, index) => ({
        ...dt, no_urut: index + 1
      }))
    }));
    ...
    if (status === 'POST') {
      ...
      const postRes = await fetch(`${store.server.url_backend}/operation/t_credit_note/post`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `${store.user.token_type} ${store.user.token}`
...
async function progress(status) {
  swal.fire({
    ...
    if (res.isConfirmed) {
      try {
        const dataURL = `${store.server.url_backend}/operation/t_credit_note/progress`
        isRequesting.value = true
...
const landing = reactive({
    ...
    // },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_credit_note/post`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_credit_note/send_approval`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
  ...
  api: {
    ...
    params: {
      join: true,
      simplest: true,
      searchfield: 'no_credit_note, pic.nama, m_perkiraan_akun_penyusutan.nama_coa',
    },
    onsuccess(response) {
  ...
  {
    headerName: 'No Credit Note',
    field: 'no_credit_note',
    filter: true,
    sortable: true,
    flex: 1,
    filter: 'ColFilter',
  ...
  {
    headerName: 'Tipe Credit Note',
    field: 'tipe_credit_note.deskripsi',
    filter: true,
    sortable: true,
    filter: 'ColFilter',
    resizable: true,
