...
const modulPath = 't_bkm_non_order'
...
onBeforeMount(async () => {
  ...
  if (isRead) {
    try {
      ...
      initialValues.t_bkm_non_order_d?.forEach((item) => {
...
async function onSave() {
    ...
    try {
        ...
        values.t_bkm_non_order_d = detailsWithSeq.map(detail => ({ ...detail}));
        ...
        const res = await fetch(dataURL, {
            ...
            body: JSON.stringify(Object.assign({}, values, {
                t_bkm_non_order_d: values.t_bkm_non_order_d
            }))
        });
        if (!res.ok) {
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_bkm_non_order/post`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
    ...
    },
    {
      ...
      async click(row) {
        try {
          ...
          window.open(`${store.server.url_backend}/web/bkm_non_order?id=${row.id}&user=${userId}`)
        } catch (err) {
          isBadForm.value = true;
          swal.fire({
            icon: 'error',
