...
onBeforeMount(() => {
  document.title = 'Transaksi Tagihan Konsolidator'
})
...
const addDetail1 = () => {
  const tempItem = {
    ...
    tarif: 0,
    is_ppn: false,
    persentase_konsolidator_jasa: 0,
  };
          ...
          detail.tarif.forEach((tarif) => {
            if (Array.isArray(tarif.jasa)) {
              tarif.jasa.forEach((jasa) => {
                ...
                jasa.persentase_konsolidator_jasa = jasa.persentase_konsolidator_jasa || 0;
...
async function generateTotal() {
  ...
  try {
    ...
    const payload = {
      ...
      persentase_konsolidator_kont: values.persentase_konsolidator_kont || false,
      grand_total_nota_rampung: grandTotalNotaRampung
    };
    console.log(payload)

    const dataURL = `${store.server.url_backend}/operation/${endpointApi}/calculate_tagihan_konsolidator`;
    ...
    // values.total_jasa_angkutan = (hasil.t_tagihan_d_tarif || []).reduce((acc, item) => {
    //   return acc + (item.tarif * (item.persentase_konsolidator_jasa / 100));
    // }, 0);

    console.log(values.total_lain_non_ppn, 'INI HASIL NON PPN')
    console.log(values.total_jasa_angkutan, 'INI HASIL')
    swal.fire({
      icon: 'success',
...
const landing = reactive({
  ...
  },
  {
    ...
    field: 'persentase_konsolidator_kont',
    filter: true,
    sortable: true,
    flex: 1,
    filter: 'ColFilter',
