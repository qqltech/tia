...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_customer_group'
onBeforeMount(() => {
  document.title = 'Master Customer Group'
})
...
onBeforeMount(async () => {
  ...
  //FETCH DETAIL
  await fetchData(`${store.server.url_backend}/operation/m_customer`, {
    where: `m_customer_group_id=${values.id}`, order_by: "created_at", order_type: "ASC"
  }).then((res) => {
    detailArr.value = res.data.map(item => (
      {
        ...item
      }
    ));
...
// PERHITUNGAN TOTAL KREDIT
const totalKreditTerpakai = computed(() => {
  let total = 0;
  detailArr.value.forEach((dt) => {
    total += parseFloat(dt.coa_piutang || 0);
  });
  return total;
});

// PERHITUNGAN SISA KREDIT
const sisaKredit = computed(() => {
  return (values.kredit_limit - values.total_kredit || 0);
});
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield: 'this.id, this.kode, this.nama, this.kredit_limit, this.sisa_kredit, this.is_active',
    },
    onsuccess(response) {
