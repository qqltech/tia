...
// ENDPOINT API
const endpointApi = 't_buku_penyesuaian'
onBeforeMount(() => {
  document.title = 'Transaction Buku Penyesuaian'
})
...
function filterShowData(statusLabel = null, noBtn = null) {
  ...
  if (valLand.filter_tahun) {
    filters.push(`EXTRACT(YEAR FROM this.tanggal_buku_penyesuaian) = ${valLand.filter_tahun}`)
  }
...
const table = reactive({
  api: {
    ...
    params: {
      simplest: true,
      searchfield: 'this.id, this.no_draft, this.total_amt, this.no_po',
    },
    onsuccess(response) {
  ...
  {
    headerName: 'tgl. BP',
    field: 'tanggal_buku_penyesuaian',
    filter: true,
    sortable: true,
    flex: 1,
    filter: 'ColFilter',
...
// FORM DATA
let default_value = {
  data: { status: 'DRAFT', no_draft: 'Generate by System', no_bkk: 'Generate by System', tipe_bkk: 'Non Kasbon' },
  detail: []
}
...
onBeforeMount(async () => {
  if (actionText.value === 'Create' || data.status === 'DRAFT') {
    data.tanggal_buku_penyesuaian = getCurrentDateFormatted();
  }
  ...
  try {
    ...
    await fetchData(dataURL, { join: true, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_buku_penyesuaian_d);
    ...
    const dataURLBKK = `${store.server.url_backend}/operation/t_bkk/${data.no_bkk_id}`;
    await fetchData(dataURLBKK, { join: true, transform: false }).then((res) => {
      // default_value.data = res.data;
      if (res.data) {
      ...
      try {
        ...
        if (isCreating) {
          ...
          const res = await fetch(dataURL, {
            ...
            body: JSON.stringify({
              ...data,
              t_buku_penyesuaian_d: detailArr,
            }),
          });
          if (res.ok) {
...
watch(() => [data.total_amt], () => {
  data.total_amt_tampil = data.total_amt.toLocaleString('id-ID');
});
//  @endif | --- END --- |
