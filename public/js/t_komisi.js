...
// ENDPOINT API
const endpointApi = 't_komisi'
onBeforeMount(() => {
  document.title = 'Transaction Komisi'
})
...
// FORM DATA
let default_value = {
  data: { status: 'DRAFT', no_komisi: 'Generate by System' },
  detail: []
}
...
onBeforeMount(async () => {
  ...
  try {
    ...
    await fetchData(dataURL, { join: true, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_komisi_d);
      for (const key in res.data) {
        data[key] = res.data[key];
      }
        ...
        const resSimpan = await fetch(dataURLSimpan, {
          ...
          if (!res.ok) {
            if ([400, 422, 500].includes(res.status)) {
              const responseJson = await res.json()
              formErrors.value = responseJson.errors || {}
              throw (responseJson.message + " " + responseJson.data.errorText || "Failed when trying to Send Post")
            } else {
              throw ("Failed when trying to Send Post")
            }
          }
          const responseJson = await res.json()
          swal.fire({
...
watch(() => detailArr, () => {
  data.total_amt = 0;
  for (let idx = 0; idx < detailArr.length; idx++) {
    data.total_amt += detailArr[idx].nominal;
  }
}, { deep: true })
