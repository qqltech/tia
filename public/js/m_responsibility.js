...
// ENDPOINT API
const endpointApi = 'm_responsibility'
onBeforeMount(() => {
    document.title = 'Master Responsibility'
})
...
const table = reactive({
    api: {
        ...
        params: {
            simplest: true,
            searchfield: 'this.id, this.nama, this.catatan',
        },
        onsuccess(response) {
    ...
    columns: [
        ...
        {
            headerName: 'Nama Responsibility', field: 'nama', flex: 1, cellClass: ['border-r', '!border-gray-200', 'justify-start',],
            sortable: true, filter: 'ColFilter',
        },
        {
            headerName: 'Catatan', field: 'catatan', flex: 2, cellClass: ['border-r', '!border-gray-200', 'justify-start'],
            sortable: true, filter: 'ColFilter', wrapText:true,
        },
        {
...
onBeforeMount(async () => {
    ...
    try {
        ...
        // FETCH DETAIL DATA 
        await fetchData(`${store.server.url_backend}/operation/m_responsibility_d`, {
            where: `m_responsibility_id=${editedId} AND m_role.is_active='true'`, order_by: "created_at", order_type: "ASC"
        }).then((res) => {
          default_value.detail = res.data.map(item => (
             {...item, name: item['m_role.name'], is_superadmin: item['m_role.is_superadmin'],
              text_superadmin: (item['m_role.is_superadmin'] === true) ? 'Yes' : 'No', 
             }
          ));
...
// ADD & DELETE  DETAIL (VALIDATION)
const addDetail = (rows) => {
    const itemsToAdd = Array.isArray(rows) ? rows : [rows];
    detail.data = [...detail.data, ...itemsToAdd];
}
...
async function onSave() {
    ...
    try {
        ...
        const res = await fetch(dataURL, {
            ...
            body: JSON.stringify({
                ...data,
                m_responsibility_d: detail.data,
