...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_grup_head'
onBeforeMount(()=>{
  document.title = 'Master Grup Head'
})
...
const addDetailArr = (params) => {
  const fixed = params.map(dt => ({
    no_head_id: dt.id,
    kode: dt.kode,
    deskripsi: dt.deskripsi,
  }))
  detailArr.value.push(...fixed)
...
onBeforeMount(async () => {
  if (isRead) {
    try {
      ...
      // Jukuk kabeh no_head_id
      const ids = initialValues.m_grup_head_d.map(dt => dt.no_head_id)

      // jukuk data referensi m_general (kode + deskripsi)
      const refRes = await fetch(`${store.server.url_backend}/operation/m_general?where=this.id IN (${ids.join(',')})`, {
        headers: {
          'Content-Type': 'Application/json',
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
      ...
      const refMap = Object.fromEntries(refJson.data.map(r => [r.id, r]))

      // mapping detailArr
      detailArr.value = initialValues.m_grup_head_d.map(dt => ({
        ...dt,
        kode: refMap[dt.no_head_id]?.kode ?? '-',
        deskripsi: refMap[dt.no_head_id]?.deskripsi ?? '-'
      }))

      console.log('IKI LOH! :', JSON.parse(JSON.stringify(detailArr.value)))
  ...
  //values.tags = JSON.stringify(values.tags)
    values.is_active = (values.is_active === true) ? 1 : 0;
      try {
        ...
        isRequesting.value = true
        values.m_grup_head_d = detailArr.value
