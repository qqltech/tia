...
// ------------------------------ PERSIAPAN
const endpointApi = 'm_tarif_angkutan'
onBeforeMount(() => {
  document.title = 'Master Tarif Angkutan'
})
...
onBeforeMount(async () => {
  ...
  if (isRead) {
    try {
      ...
      initialValues.nama = initialValues['m_supplier.nama']
      // initialValues.kena_pajak = initialValues.kena_pajak? 1 : 0
    } catch (err) {
  ...
  for (const key in initialValues) {
    ...
    try {
      ...
      const params = {
        ...
        transform: false,
        join: false,
        where: `this.group='JENIS PPH' and this.is_active=true`
      }
      const fixedParams = new URLSearchParams(params)
      const res = await fetch(dataURL + '?' + fixedParams, {
        headers: {
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield: 'this.id, this.kode, m_supplier.kode, m_supplier.nama, this.catatan'
    },
