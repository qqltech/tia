...
// ------------------------------ PERSIAPAN
const endpointApi = 't_asset_disposal'
onBeforeMount(()=>{
  document.title = 'Transaksi Asset Disposal'
})
...
async function onSave() {
    ...
    try {
        ...
        values.t_asset_disposal_d = detailsWithSeq.map(detail => ({ ...detail}));
        ...
        const res = await fetch(dataURL, {
            ...
            body: JSON.stringify(Object.assign({}, values, {
                t_asset_disposal_d: values.t_asset_disposal_d
            }))
        });
        if (!res.ok) {
