...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_asset'
onBeforeMount(()=>{
  document.title = 'Master Asset'
})
...
onBeforeMount(async () => {
  if (isRead) {
    ...
    try {
      ...
      // Detail 
      initialValues.m_asset_d?.forEach((item) => {
...
const onDetailAdd = (data) => {
  data.forEach(row => {
    console.log('check',row.nama)
    row.nama_supplier = row.nama
    row.m_supplier_id = row.id
...
async function onSave() {
  //values.tags = JSON.stringify(values.tags)
    values.is_active = (values.is_active === true) ? 1 : 0;
      try {
        values.m_asset_d= detailArr.value;
