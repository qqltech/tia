...
class Approval
{
    ...
    protected function checkUserCanCreateTicket($m_approval)
    {
        ...
        $fixedConfig =
            "select d.* from set.m_approval_det d join set.m_approval a on a.id = d.m_approval_id where a.id= ? AND tipe = 'MENGAJUKAN' ORDER BY d.level ASC";
        ...
        if (
            $detail->default_user_id &&
            $detail->default_user_id !== $userAuth->id
        ) {
            $text =
                "Anda tidak memiliki akses untuk membuat tiket approval ini";
            $next = false;
            $kode = "0000003";
        }
    ...
    protected function checkUserCanApprove($approval_det)
    {
        ...
        $check = generate_approval::whereRaw(
        ...
        $check_log = generate_approval_log::where(
            "generate_approval_det_id",
            $approval_det->id
        )->exists();
    ...
    public function approvalCreateTicket(
        array $config,
        bool $errorOnFailed = false
    ) {
        ...
        $m_approval = m_approval_det::join(
            "set.m_approval as a",
            "a.id",
            "m_approval_det.m_approval_id"
        )
        ...
        $check_approve_atasan = m_approval_det::join(
            "set.m_role",
            "m_role.id",
            "m_approval_det.m_role_id"
        )
        ...
        if (strtolower(@$check_approve_atasan->name) == "atasan") {
            $atasan_id =
                m_kary::join("default_users as u", "u.m_kary_id", "m_kary.id")
                    ->where("u.id", auth()->user()->id)
                    ->pluck("atasan_id")
                    ->first() ?? 0;
            $user_atasan_id = default_users::whereRaw(
                "default_users.m_kary_id = $atasan_id"
            )
                ->orderBy("id", "asc")
                ->pluck("id")
                ->first();
        ...
        try {
            ...
            $pengajuan = DB::select($fixedConfigPengajuan, [
                $m_approval->m_approval_id,
            ]);
            if (count($pengajuan)) {
            ...
            $last_det_approver = generate_approval_det::where(
    ...
    public function approvalProgress(array $config, bool $errorOnFailed = false)
    {
        ...
        $app_type = $config["app_type"];
        $app_note = $config["app_note"];
        $auto = $config["auto"] ?? false; // default false jika tidak diset
        ...
        $fixedConfig =
            "select d.* from set.generate_approval_det d join set.generate_approval a on a.id = d.generate_approval_id where a.id = ? and d.is_done = false order by d.urutan_level limit 1";
        $process_data = DB::select($fixedConfig, [$app_id]);
        ...
        // Hanya lakukan checkUserCanApprove jika auto = false
        if (!$auto) {
            $this->checkUserCanApprove($process_data);
        }
        ...
        try {
            ...
            generate_approval_det::whereRaw($whereRawUpdate)->update([
                "action_at" => $this->timestamp,
                "action_type" => $app_type,
                "action_user_id" => auth()->user()->id,
                "action_note" => $app_note,
            ...
            $outstanding = generate_approval_det::where(
                "generate_approval_id",
                $generate_approval->id
            )
                ->where("is_done", false)
                ->where("urutan_level", ">", $process_data->urutan_level)
                ->exists();
            ...
            if ($outstanding && $app_type != "REJECTED") {
                ...
                generate_approval_det::where("id", $last_det_approver)->update([
                    "assigned_at" => $this->timestamp,
                ]);
                generate_approval::find($generate_approval->id)->update([
                    "last_approve_id" => null,
                ]);
            } else {
                $finish = true;
                generate_approval::find($generate_approval->id)->update([
            ...
            $log_insert = generate_approval_log::create([
                "nomor" => $generate_approval->nomor,
                "generate_approval_id" => $generate_approval->id,
                "generate_approval_det_id" => @$process_data->id ?? 0,
                "trx_id" => $generate_approval->trx_id,
                "trx_table" => $generate_approval->trx_table,
                "trx_name" => $generate_approval->trx_name,
                "trx_nomor" => $generate_approval->trx_nomor,
    ...
    public function approvalOustanding()
    {
        ...
        $data = generate_approval::selectRaw(
            "generate_approval.*,(select u.name from default_users u where u.id = generate_approval.creator_id) creator"
        )
            ->leftJoin(
                "default_users",
                "default_users.id",
                "generate_approval.creator_id"
            )
            ->whereRaw(
                "generate_approval.status = 'PROGRESS' 
                and case when generate_approval.last_approve_id is not null then
    ...
    {
        $app = DB::table("set.generate_approval")
            ->selectRaw(
                "*,(select u.name from default_users u where u.id = generate_approval.creator_id) creator"
            )
            ->where("id", $id)
            ->first();
        ...
        $app->tahap_saat_ini =
            DB::table("set.generate_approval_det")
                ->where("generate_approval_id", $app->id)
                ->where("is_done", false)
                ->orderBy("urutan_level", "asc")
                ->pluck("urutan_level")
                ->first() ?? 0;
        $app->tahap_total = DB::table("set.generate_approval_det")
            ->where("generate_approval_id", $app->id)
            ->count();

        $trx = DB::table($app->trx_table)
            ->where("id", $app->trx_id)
            ->first();

        $data = new stdClass();
        $data->approval = $app;
        $data->approval_log = $this->approvalLog([
            "trx_id" => $app->trx_id,
    ...
    public function approvalLog($conf)
    {
        ...
        $data = generate_approval_log::selectRaw(
            "*,(select u.name from default_users u where u.id = generate_approval_log.action_user_id) action_user"
        )
