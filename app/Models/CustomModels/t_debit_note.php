   ...
   private $approval;
    ...
    public function createBefore($model, $arrayData, $metaData, $id = null)
    {
        $newData = [
            ...
            "no_debit_note" => $this->helper->generateNomor("No Debit Note"),
            "tanggal" => date("Y-m-d"),
            "status" => "DRAFT",
        ];
    ...
    {
        $general = m_general::where('id',$arrayData['tipe_debit_note'])->first()->deskripsi ?? '';
    ...
    {
        $tempId = $id;
        $trx = \DB::table('t_debit_note')->find($tempId);
        $conf = [
            "app_name" => "APPROVAL DEBIT NOTE",
            "trx_id" => $trx->id,
            "trx_table" => $this->getTable(),
            "trx_name" => "Pengajuan Debit Note",
            "form_name" => "t_debit_note",
            "trx_nomor" => $trx->no_debit_note,
            "trx_date" => Date("Y-m-d"),
