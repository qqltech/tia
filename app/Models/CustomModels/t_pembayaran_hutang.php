...
{    
    ...
    public function custom_send_approval()
    {
        ...
        if (app()->request->header("Source") != "mobile") {
            $spd = t_pembayaran_hutang::find(req("id"));
            if ($spd) {
                $spd->update([
                    "status" => "IN APPROVAL",
                ]);
    ...
    {
        $tempId = $id;
        $trx = \DB::table('t_pembayaran_hutang')->find($tempId);
        $conf = [
            "app_name" => "APPROVAL PEMBAYARAN HUTANG",
            "trx_id" => $trx->id,
            "trx_table" => $this->getTable(),
            "trx_name" => "Pengajuan Pembayaran Hutang",
            "form_name" => "t_pembayaran_hutang",
            "trx_nomor" => $trx->no_pembayaran,
            "trx_date" => Date("Y-m-d"),
