...
class t_angkutan extends \App\Models\BasicModels\t_angkutan
{
    ...
    public function custom_Party2($id)
    {
        $container = \DB::table("t_buku_order_d_npwp")
            ->leftJoin(
                "set.m_general as mg",
                "mg.id",
                "t_buku_order_d_npwp.ukuran"
            )
            ->select(
                \DB::raw(
                    'COUNT("ukuran") as jumlah,mg.deskripsi as ukuran_value'
                )
            )
    ...
    public function custom_detailbo_container()
    {
        ...
        // ->get();
        $nama_angkutan_tia = \DB::table("m_supplier as ms")
            ->where("ms.nama", "TIA SENTOSA MAKMUR")
            ->first();
        ...
        $result = \DB::table("t_buku_order_d_npwp as tbodn")
            ...
            ->leftJoin(
                "t_spk_angkutan as tsa",
                "tsa.t_detail_npwp_container_1_id",
                "=",
                "tbodn.id"
            )
            ->leftJoin(
                "t_spk_angkutan as tsa2",
                "tsa2.t_detail_npwp_container_2_id",
                "=",
                "tbodn.id"
            )
            ...
            ->select(
                ...
                \DB::raw(
                    "to_char(tbo.tanggal_pengkont,'DD/MM/YYYY') as tanggal_pengkont_new"
                ),
    ...
    public function custom_getStaple()
    {
        ...
        $isSpecialCase = $req->is_special_case;
        ...
        $day = strval($this->minusDate($tanggal_in, $tanggal_out));
