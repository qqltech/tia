...
class t_nota_rampung extends \App\Models\BasicModels\t_nota_rampung
{
    ...
    {
        $idBukuOrder = @$id ?? 0;
        $getNotaRampung = t_nota_rampung::where(
            "t_buku_order_id",
            $idBukuOrder
        )->get();
        $grandTotal = 0;
        foreach ($getNotaRampung as $single) {
            $total =
                ...
                $single["strp_stuf"] +
                $single["canc_doc"] +
                $single["closing_container"] +
                $single["batal_muat"] +
                $single["vgm"] +
                $single["mob"] +
                $single["denda_koreksi"] +
                $single["materai"] +
                $single["by_adm_nr"] +
                $single["denda_sp"];
    ...
    function custom_get_bo_d_npwp()
    {
        ...
        $result = \DB::table("t_buku_order_d_npwp as tbodn")
            ...
            ->select(
                ...
                "tbodn.*",
                \DB::raw(
                    "CONCAT(tbodn.no_prefix,'-',tbodn.no_suffix) as no_container"
                )
            )
            ->where("tbo.id", $id)
            ->get();
    ...
    // {
    //     $grandTotal = 0;
    //     $pelabuhan_id = $req['t_nota_rampung_d'][0]['t_buku_order.pelabuhan_id'];
    //     $tipe_nota_rampung_input = $req['tipe1'];
    //     $arrayContainer = $req['t_nota_rampung_d'];

    //     $tipeTarifAktual = \DB::table('set.m_general')
    //         ->where('group', 'TIPE TARIF NOTA RAMPUNG')
    //         ->where(function ($q) use ($tipe_nota_rampung_input) {
    //             $q->where('id', $tipe_nota_rampung_input)
    //             ->orWhere('kode', $tipe_nota_rampung_input)
    //             ->orWhere('deskripsi', $tipe_nota_rampung_input);
    //         })
    //         ->value('id') ?? $tipe_nota_rampung_input; // fallback kalau tidak ketemu

    //     foreach ($arrayContainer as $single) {

    //         $getTarif = \DB::table('m_tarif_nota_rampung as tnr')
    //             ->join('set.m_general as pl', 'pl.id', '=', 'tnr.kode_pelabuhan')
    //             ->join('set.m_general as uk', 'uk.id', '=', 'tnr.ukuran_container')
    //             ->join('set.m_general as jk', 'jk.id', '=', 'tnr.jenis_container')
    //             ->join('set.m_general as tt', 'tt.id', '=', 'tnr.tipe_tarif')
    //             ->select(
    //                 'tnr.*',
    //                 'pl.deskripsi as nama_pelabuhan',
    //                 'uk.deskripsi as ukuran_name',
    //                 'jk.deskripsi as jenis_name',
    //                 'tt.deskripsi as tipe_name'
    //             )
    //             ->where('tnr.kode_pelabuhan', $pelabuhan_id)
    //             ->where('tnr.ukuran_container', $single['ukuran'])
    //             ->where('tnr.jenis_container', $single['jenis'])
    //             ->where('tnr.tipe_tarif', $tipeTarifAktual)
    //             ->first();

    //         \Log::info('Cek tarif NR', [
    //             'pelabuhan' => $pelabuhan_id,
    //             'ukuran' => $single['ukuran'],
    //             'jenis' => $single['jenis'],
    //             'tipe_dikirim' => $tipe_nota_rampung_input,
    //             'tipe_aktual' => $tipeTarifAktual,
    //             'found' => $getTarif ? 'YES' : 'NO'
    //         ]);
    ...
    //         foreach ($fields as $tarifField => $inputField) {
    //             $grandTotal += ($getTarif->$tarifField ?? 0) * ($single[$inputField] ?? 0);
    //         }
    //     }
