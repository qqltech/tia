    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_purchase_order/send_approval`
...
async function loadHistoryStock(data) {
  ...
  const item_id = data['m_item.id']
  const url = `${store.server.url_backend}/operation/t_pemakaian_stok_d?where=this.m_item_id=${item_id}`
  const res = await fetch(url, {
    headers: {
      'Content-Type': 'Application/json',
      Authorization: `${store.user.token_type} ${store.user.token}`
...
watch(() => detailArr, () => {
  ...
  if (data.ppn == "INCLUDE") data.total_ppn = data.dpp * 0.11;
  else data.total_ppn = data.total_amount * 0;
