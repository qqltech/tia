    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_spk_angkutan/send_approval`
...
console.log(trx_id)
    ...
    console.log(data.t_detail_npwp_container_1_id);
...
async function progress(status) {
  swal.fire({
    ...
    if (res.isConfirmed) {
      try {
        const dataURL = `${store.server.url_backend}/operation/t_spk_angkutan/progress`
        isRequesting.value = true
        const res = await fetch(dataURL, {
          method: 'POST',
          headers: {
