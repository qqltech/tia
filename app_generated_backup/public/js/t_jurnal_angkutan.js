...
async function supplier(m_supplier_id) {
  console.log('babi',m_supplier_id)

  // detailArr.value = [];
  // detailArr1.value = [];
  // detailArr2.value = [];
  // detailArr3.value = [];
  // values.grand_total_nota_rampung = null;
  // if (!no_buku_order || !no_buku_order.id) {
  //   values.customer = ''; 
  //   values.ppn = null;
  //   values.total_amount = null;
  //   values.grand_total_amount = null;
  //   return; 
  // }
...
values = reactive({
  status : "DRAFT" ,
  tgl : new Date().toLocaleDateString('id-ID', { month: '2-digit', day: '2-digit', year: 'numeric' })
})
...
async function onSave() {
  //values.tags = JSON.stringify(values.tags)
    try {
      ...
      if (!isCreating) {
          values.tgl = new Date().toISOString().split('T')[0]
        }
