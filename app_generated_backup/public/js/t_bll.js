...
// ENDPOINT API
const endpointApi = 't_bll'
onBeforeMount(() => {
  document.title = 'Transaction BLL'
})
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_bll/post`
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_bll/approval`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
...
// FORM DATA
let default_value = {
  data: { status: 'DRAFT', no_draft: 'Generate by System', no_bll: 'Generate by System' },
  detail: []
}
...
onBeforeMount(async () => {
  ...
  try {
    ...
    await fetchData(dataURL, { join: false, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_bll_d);
