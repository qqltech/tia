    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_rencana_pembayaran_hutang/posted`
...
// FORM DATA
let default_value = {
  data: { status: 'DRAFT', persen_ppn: 0, persen_pph: 0, total_amount: 0, dpp: 0, total_ppn: 0, grand_total: 0, no_draft: 'Generate by System', no_pi: 'Generate by System' },
  detail: []
}
...
onBeforeMount(async () => {
  ...
  try {
    ...
    await fetchData(dataURL, { join: false, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_rencana_pembayaran_hutang_d);
...
const poChanged = async (id) => {
  ...
  const queryString = new URLSearchParams({ simplest: true, where: `this.t_no_po_id=${id}` }).toString();
...
watch(() => detailArr, () => {
  ...
  data.nilai_ppn = data.total_amount * data.persen_pph;
  data.nilai_pph = data.total_amount * data.persen_pph;
  data.jumlah = data.total_amount + data.nilai_ppn + data.nilai_pph;

}, { deep: true })

watch(() => [data.persen_ppn, data.persen_pph], () => {
  data.nilai_ppn = data.total_amount * data.persen_ppn;
  data.nilai_pph = data.total_amount * data.persen_pph;
  data.jumlah = data.total_amount + data.nilai_ppn + data.nilai_pph;
});
