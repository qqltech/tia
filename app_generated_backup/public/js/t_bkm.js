...
const endpointApiNonOrder = 't_bkm_non_order'
onBeforeMount(() => {
...
let initialValues = { tipe: route.query.tipe }
const changedValues = []
...
onBeforeMount(async () => {
  ...
  if (isRead) {
    ...
    try {
      ...
      let dataURL;
      if (tipe.value = "BKM") dataURL = `${store.server.url_backend}/operation/${endpointApi}${isCreating ? '' : ('/' + route.params.id)}`;
      else if (tipe.value = "BKM Non Order") dataURL = `${store.server.url_backend}/operation/${endpointApiNonOrder}${isCreating ? '' : ('/' + route.params.id)}`;
