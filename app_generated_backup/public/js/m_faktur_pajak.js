...
const landing = reactive({
    api: {
        ...
        params: {
            simplest: true,
            searchfield: `this.id, this.start_date, this.end_date, this.prefix`
        },
        onsuccess(response) {
    ...
    columns: [
        ...
        {
            headerName: 'Prefix', field: 'prefix', flex: 1, cellClass: ['border-r', '!border-gray-200', 'justify-start'],
            sortable: true, filter: 'ColFilter'
        }
    ],
    actions: [
...
function generateDetail(){
  ...
  const newData = Array.apply(null, Array(data.no_akhir - data.no_awal + 1)).map(() => {
    const genData =  {no_faktur_pajak: `${data.prefix}${current_num}`, t_nota_id: 1, is_active: 'OPEN'}
    current_num++;
    return genData;
  })
