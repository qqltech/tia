...
// ENDPOINT API
const endpointApi = 't_bkk_non_order'
onBeforeMount(() => {
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_bkk_non_order/post`
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_bkk_non_order/approval`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
...
// FORM DATA
let default_value = {
  data: { status: 'DRAFT', no_draft: 'Generate by System', no_bkk: 'Generate by System' },
  detail: []
}
...
onBeforeMount(async () => {
  ...
  try {
    ...
    await fetchData(dataURL, { join: false, transform: false }).then((res) => {
      ...
      detailArr.push(...res.data.t_bkk_non_order_d);
