...
// ------------------------------ PERSIAPAN
const endpointApi = '/t_pembayaran_piutang/customer'
onBeforeMount(() => {
...
onMounted(() => {
  ...
  values.tanggal_pembayaran = formattedDate;
})
...
onBeforeMount(async () => {
  ...
  if (isRead) {
    ...
    try {
      ...
      const params = { id: editedId, join: false, transform: false }
      ...
      // // Logika untuk mengubah nilai status
      // for (const key in initialValues) {
      //   if (key === 'status') {
      //     if (initialValues[key] === null) {
      //       initialValues[key] = 'DRAFT';
      //     } else if (initialValues[key] === 'DRAFT' && initialValues.hasOwnProperty('data')) {
      //       initialValues[key] = 'In Process';
      //     }
      //   }
      // }
...
async function onSave() {
  ...
  try {
    ...
    const dataURL = `${store.server.url_backend}/operation${endpointApi}?id=${route.params.id}`
    isRequesting.value = true
  ...
  },
  {
    ...
    cellRenderer: ({ value }) =>
      value === 'DRAFT' 
        ...
        : value === null ? `<span class="text-red-500 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">Belum Dibuat</span>`
