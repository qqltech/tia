...
{    
    ...
    public function transformRowData( array $row )
    {
        ...
        if(app()->request->view_tarif){
            ...
            $getTarif = $this->tarif_kontainer($getBukuOrder['m_customer_id'], $row['ukuran'],ucfirst(strtolower($getBukuOrder['tipe_order'])),$row['jenis'])->toArray();
            $data = [
                "tarif" => $getTarif
            ];
    ...
    public function tarif_kontainer($getCustomerId, $ukuran_id, $tipe_tarif, $jenis){
        ...
        $getIdUkuran = \DB::table('m_tarif as mt')
            ...
            ->where('mg.deskripsi', $ukuran_id)
            ->where('mt.is_active', true)
            ->where('mt.tipe_tarif', $tipe_tarif)
