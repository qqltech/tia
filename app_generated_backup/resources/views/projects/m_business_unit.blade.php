  ...
  <div class="bg-blue-500 text-white rounded-t-md pt-2 pb-3 px-4">
    ...
    <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 sticky">
        <div class="relative">
          ...
          <FieldX :bind="{ readonly: true || !actionText }" class="pt-1" :value="data.kode" :errorText="formErrors.kode?'failed':''" @input="v=>data.kode=v" :hints="formErrors.kode" label="Kode" placeholder='Auto Generate by System' :check="false" />
        </div>
        ...
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.alamat" :errorText="formErrors.alamat?'failed':''" 
        @input="v=>data.alamat=v" :hints="formErrors.alamat" placeholder="Alamat" :check="false" />
        ...
        <FieldSelect :bind="{ disabled: (!data.provinsi) || !actionText, clearable:false }" class="pt-1" 
        :value="data.kota" @input="v=>data.kota=v" :errorText="formErrors.kota?'failed':''" :hints="formErrors.kota" 
        valueField="key" displayField="key" :api="{
        ...
        <FieldSelect :bind="{ disabled: (!data.kota) || !actionText, clearable:false }" class="pt-1" :value="data.kecamatan" @input="v=>data.kecamatan=v" :errorText="formErrors.kecamatan?'failed':''" :hints="formErrors.kecamatan" valueField="key" displayField="key" :api="{
        ...
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.kodepos" :errorText="formErrors.kodepos?'failed':''" 
        @input="v=>data.kodepos=v" :hints="formErrors.kodepos" placeholder="Kode Pos" :check="false" type="number" />
    <div class="relative">
