...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
  ...
  <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 items-start">
    ...
    <div>
      ...
      }" displayField="no_spk" valueField="id" :bind="{ readonly: !actionText }"
        :value="data.t_detail_npwp_container_1_id" @input="(v)=>data.t_detail_npwp_container_1_id=v" @update:valueFull="(res)=>{
        $log(res);
          data.no_container = res.no_container_1;
          data.no_order = res['t_buku_order_1.no_buku_order'];
          data.no_angkutan = res['t_buku_order_1.angkutan'];
          ...
          data.waktu_out = res.waktu_out;
          data.ukuran_container = res['t_detail_npwp_container_1.ukuran'];
          data.m_karyawan_id = res['supir.id'];
          data.chasis = res.chasis;
          data.total_sangu = res.sangu;
          data.sektor = res.sektor;
          data.ke = res.ke;
          data.dari = res.dari;


          data.m_tarif_premi_id = res.m_tarif_premi_id;
    ...
    <div>
      <FieldSelect class="w-full !mt-3" :bind="{ disabled: true, clearable:true }" :value="data.status"
        ...
        displayField="key" :options="[{'id' : 'In Process' , 'key' : 'In Process'},
      {'id' : 'In Process', 'key' : 'In Process'},]" placeholder="Pilih Status" label="Status" :check="false" />
    </div>
    <div>
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.no_order" @input="v=>data.no_order=v"
        :errorText="formErrors.no_order?'failed':''" :hints="formErrors.no_order" label="No. Container"
    ...
    <div>
      <FieldX :bind="{ readonly: true }" class="w-full !mt-3" :value="data.no_bon_sementara"
        @input="v=>data.no_bon_sementara=v" :errorText="formErrors.no_bon_sementara?'failed':''"
        :hints="formErrors.no_bon_sementara" label="No. Bon Sementara" placeholder="Masukan No. Bon Sementara"
        :check="false" />
    </div>
    <div class="grid grid-cols-2 gap-y-2 gap-x-2 items-start">
      <FieldX :bind="{ disabled: true, readonly: true }" class="w-full !mt-3" :value="data.tanggal_bon"
        :errorText="formErrors.tanggal_bon?'failed' :''" @input="v=>data.tanggal_bon=v" :hints="formErrors.tanggal_bon"
        :check="false" type="date" label="Tanggal Bon" placeholder="Pilih Tanggal Bon" />
    </div>
    <div class="grid grid-cols-2 gap-y-2 gap-x-2 items-start">
    ...
    <div>
      <FieldPopup class="w-full !mt-3" :api="{
        url: `${store.server.url_backend}/operation/m_karyawan`,
        headers: {
          'Content-Type': 'Application/json',
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
    ...
    <div>
      <FieldSelect class="w-full !mt-3" :bind="{  disabled: true, readonly: true }" :value="data.chasis"
        @input="v=>data.chasis=v" :errorText="formErrors.chasis?'failed':''" :hints="formErrors.chasis" valueField="id"
        displayField="key" :options="[{'id' : 'Chasis A' , 'key' : 'Chasis A'},
      {'id' : 'Chasis B', 'key' : 'Chasis B'},
      {'id' : 'Chasis C', 'key' : 'Chasis C'}]" placeholder="Pilih Chasis" label="Chasis" :check="false" />
    </div>
    <div>
      <FieldX type="number" :bind="{ readonly: true }" class="w-full !mt-3" :value="data.total_sangu"
    ...
    <div>
      <FieldX type="number" :bind="{ readonly: true }" class="w-full !mt-3" :value="data.sektor"
        @input="v=>data.sektor=v" :errorText="formErrors.sektor?'failed':''" :hints="formErrors.sektor"
        placeholder="Sektor" :check="false" />
    </div>
    <div>
    ...
    <div>
      ...
      }" displayField="no_tarif_premi" valueField="id" :bind="{ readonly: true }" :value="data.m_tarif_premi_id"
        @input="(v)=>data.m_tarif_premi_id=v" @update:valueFull="(response)=>{
        $log(response);
    ...
    <div>
      <FieldX type="number" :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="data.tol"
        @input="v=>data.tol=v" :errorText="formErrors.tol?'failed':''" :hints="formErrors.tol" placeholder="Tol"
        label="Tol" :check="false" />
    </div>
    <div>
