...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
  ...
  <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 items-start">
    ...
    <div>
      <FieldPopup class="w-full !mt-3" :api="{
        ...
        :hints="formErrors.t_purchase_order_id" label="Copy PO" placeholder="Pilih PO" :check='false' :columns="[
        {
          headerName: 'No',
    ...
    <div>
      ...
      {'id' : 'NOT INC', 'key' : 'NOT INCLUDE'}]" placeholder="PPN" label="PPN" fa-icon="sort-desc" :check="false" />
    </div>
    ...
    <div>
      <FieldX :bind="{ readonly: actionText!='Edit' }" class="w-full !mt-3" :value="data.alasan_revisi"
        ...
        <tbody>
          <tr v-for="(item, i) in detailArr" :key="i" v-show="detailArr.length > 0">
            ...
            <td class="p-1 text-center border border-[#CACACA]">
              ...
              <FieldX :bind="{ readonly: true }" class="m-0" :value="detailArr[i].kode"
                @input="v=>detailArr[i].kode=v" :hints="formErrors.kode" :check="false" />
            </td>
            <td class="p-1 text-center border border-[#CACACA]">
            ...
            <td class="p-1 text-center border border-[#CACACA]">
              <FieldX type="textarea" :bind="{ readonly: !actionText }" class="m-0" :value="detailArr[i].catatan"
                @input="v=>detailArr[i].catatan=v" :hints="formErrors.catatan" :check="false" />
            </td>
  ...
  <div v-show="modalOpenHistoryStock" class="fixed inset-0 flex items-center justify-center z-50">
    ...
    <div class="modal-container bg-white  w-[50%] mx-auto rounded shadow-lg z-50 overflow-y-auto">
      <div class="modal-content py-4 text-left px-6">
        ...
        <!-- Modal Body -->
        <div v-if="dataHistoryStockItem?.items.length" class="modal-body">
          <table class="w-[100%] my-3 border">
            <thead>
              <tr class="border">
                <td class="border px-2 py-1 font-medium ">No</td>
            ...
            <tr class="border" v-for="d,i in dataHistoryStockItem?.items" :key="i">
              <td class="border px-2 py-1">{{ i+1 }}</td>
              <td class="border px-2 py-1">{{ d.['t_pemakaian_stok.no_pemakaian_stok'] ?? '-' }}</td>
              <td class="border px-2 py-1">{{ d.usage ? (d.qty).toLocaleString('id'):'-' }}</td>
            </tr>
          </table>
        </div>
        <!-- Modal Footer -->
  ...
  <div class="flex flex-row items-center justify-end space-x-2 py-3 px-4" v-show="actionText">
    ...
    <button  v-if="(actionText=='Edit' && data.status=='DRAFT')" class="text-sm rounded py-2 px-2.5 text-white bg-purple-600 hover:bg-purple-700 flex gap-x-1 items-center
