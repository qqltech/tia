...
<div class="flex flex-col border rounded-md shadow-md md:w-full w-full bg-white border-none">
    ...
    <div class="pt-2 pb-4 px-4 grid grid-cols-3 gap-y-2 gap-x-4 items-start">
        ...
        <FieldX :bind="{ readonly: !actionText }" class="pt-1" :value="data.no_ppjk" :errorText="formErrors.no_ppjk?'failed':''" 
          @input="v=>data.no_ppjk=v" :hints="formErrors.no_ppjk" placeholder="No PPJK" :check="false" />
        ...
        <FieldSelect :bind="{ disabled: !actionText, clearable: false }" class="pt-1" :value="data.currency" 
          @input="v=>data.currency=v" :errorText="formErrors.currency?'failed':''" :hints="formErrors.currency" 
          valueField="id" displayField="key" :options="[
            {'id' : 'IDR' , 'key' : 'IDR'}, 
            {'id': 'USD', 'key' : 'USD'},
          ]" 
          placeholder="Currency" :check="false" @update:value="(val)=>{
            if(val === 'IDR'){
              data.nilai_kurs = 1;
            }
          }" />
