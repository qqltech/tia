  ...
  <div class="p-4 grid <md:grid-cols-1 grid-cols-3 gap-2 ">
    ...
    <div class=" w-full !mt-3">
          <FieldSelect
            ...
            :value="values.m_spk_angkutan_id" @input="v=>values.m_spk_angkutan_id=v"
            :errorText="formErrors.m_spk_angkutan_id?'failed':''" 
            :hints="formErrors.m_spk_angkutan_id"
            valueField="id" displayField="no_spk"
            :api="{
            ...
            label="No Spk Angkutan"
            placeholder="Pilih No Spk Angkutan" 
            fa-icon="sort-desc"
            :check="false"
          />
    </div>
    ...
    <!-- Kode Jalan Coloumn -->
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.kode_jalan"
        :errorText="formErrors.kode_jalan?'failed':''" @input="v=>values.kode_jalan=v"
        :hints="formErrors.kode_jalan" label="Kode Jalan" placeholder="Kode Jalan" :check="false" />
    </div>
    ...
    <div class=" w-full !mt-3">
          <FieldSelect
            class="!mt-0"
            :bind="{ disabled: !actionText, clearable:true }"
            :value="values.tipe_kontainer" @input="v=>values.tipe_kontainer=v"
            :errorText="formErrors.tipe_kontainer?'failed':''" 
            :hints="formErrors.tipe_kontainer"
    ...
    <div>
      <FieldX :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.premi"
        :errorText="formErrors.premi?'failed':''" @input="v=>values.premi=v" :hints="formErrors.premi"
        label="Premi" placeholder="Tuliskan Premi" :check="false" />
    </div>

    <!-- Ganti Solar Premi Coloumn -->
    <div>
      <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.ganti_solar_premi"
        ...
        label="Ganti Solar Premi" placeholder="Ganti Solar Premi" :check="false" type="textarea" />
    </div>
    ...
    <div class=" w-full !mt-3">
          <FieldSelect
            class="!mt-0"
            :bind="{ disabled: !actionText, clearable:true }"
            :value="values.m_sektor_id" @input="v=>values.m_sektor_id=v"
            :errorText="formErrors.m_sektor_id?'failed':''" 
            :hints="formErrors.m_sektor_id"
            valueField="id" displayField="deskripsi"
            :api="{
                url: `${store.server.url_backend}/operation/m_general`,
                headers: { 'Content-Type': 'Application/json', Authorization: `${store.user.token_type} ${store.user.token}`},
    ...
    <div>
      <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.sangu"
        :errorText="formErrors.sangu?'failed':''" @input="v=>values.sangu=v"
        :hints="formErrors.sangu" label="Sangu" placeholder="Masukan Sangu" :check="false" />
    </div>

    <!-- Tagihan Coloumn -->
    <div>
      <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.tagihan"
        :errorText="formErrors.tagihan?'failed':''" @input="v=>values.tagihan=v"
        :hints="formErrors.tagihan" label="Tagihan" placeholder="Masukan Tagihan" :check="false" />
    </div>

    <!-- Ganti Solar Tagihan Coloumn -->
    <div>
      <FieldNumber :bind="{ readonly: !actionText }" class="w-full !mt-3" :value="values.ganti_solar_tagihan"
        :errorText="formErrors.ganti_solar_tagihan?'failed':''" @input="v=>values.ganti_solar_tagihan=v"
        :hints="formErrors.ganti_solar_tagihan" label="Ganti Solar" placeholder="Masukan Ganti Solar" :check="false" />
    </div>
